...
export const AGENDA_THREE_CARDS_GEOM = {
  viewW: 1000,
  viewH: 560,
  headingY: 28,
  headingH: 52,
...
export const AGENDA_THREE_CARDS_HERO_GEOM = {
  viewW: 1000,
  viewH: 560,
  headingY: 226,
  headingH: 40,
...
export const THREE_CARDS_PALETTE = [
...
export function agendaThreeCardsGraphicFrame(canvasW, canvasH, extra = {}) {
  if (extra.hero) {
    return {
      graphicX: 0,
      graphicY: 0,
...
export function agendaThreeCardsCardInlineSvg(spec) {
  const w = spec.w
  const h = spec.h
...
export function agendaThreeCardsChromeSpecs(extra = {}) {
...
export function agendaThreeCardsOverlayPlacements(gx, gy, gw, gh, extra = {}) {
  const geom = resolveGeom(extra.hero)
...
export function agendaThreeCardsPreviewSvg() {
...
export function specToThreeCardsContent(spec) {
  return { svg: agendaThreeCardsCardInlineSvg(spec), colorMode: 'original', fill: spec.color }
}

export function isAgendaThreeCardsLayout(layoutId, family, variant) {
  if (family === 'three_col' && variant === 'cards') return true
  return /agenda_three_cards_v1$/i.test(String(layoutId || ''))
}

export function isAgendaThreeCardsHeroLayout(layoutId, family, variant) {
