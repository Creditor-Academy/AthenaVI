/**
 * Para Title Left Image Overlay
 * Layout ID: para_title_left_image_overlay_v1
 *
 * Full-bleed photo, left cinematic fade, accent dash + heading + body.
 * Same slots as the boxed twin. Heading and body do not overlap.
 */

export function isParaTitleLeftImageOverlayLayout(layoutId, schema) {
...
export const PARA_TITLE_LEFT_IMAGE_OVERLAY_DEFAULTS = {
...
export function buildParaTitleLeftImageOverlaySvg() {
...
export function layoutParaTitleLeftImageOverlay(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildParaTitleLeftImageOverlayCanvasElements({ schema, options = {} } = {}) {
  ...
  return buildElements({
    canvasW,
    canvasH,
    headingText: String(contentBySlotId.HEADING || content.title || PARA_TITLE_LEFT_IMAGE_OVERLAY_DEFAULTS.HEADING).trim(),
    bodyText: String(contentBySlotId.BODY || content.body || PARA_TITLE_LEFT_IMAGE_OVERLAY_DEFAULTS.BODY).trim(),
    imageUrl:
...
export function paraTitleLeftImageOverlayPreviewSvg() {
  const { textX, barY, barW, barH, headingY } = GEOM
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    <defs>
      <linearGradient id="ptlioSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8" />
        <stop offset="100%" stop-color="#D0E3EF" />
      </linearGradient>
      <linearGradient id="ptlioHill" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#8FBF8E" />
        <stop offset="100%" stop-color="#5E945D" />
      </linearGradient>
    </defs>
    <rect width="1920" height="1080" fill="url(#ptlioSky)" />
    <ellipse cx="1280" cy="280" rx="110" ry="48" fill="#FFFFFF" opacity="0.94" />
    <ellipse cx="1360" cy="280" rx="80" ry="38" fill="#FFFFFF" opacity="0.94" />
    <ellipse cx="620" cy="220" rx="90" ry="40" fill="#FFFFFF" opacity="0.9" />
    <path d="M0 640 C360 520 720 620 1080 560 C1400 510 1680 620 1920 540 L1920 1080 L0 1080 Z" fill="url(#ptlioHill)" />
    ${buildParaTitleLeftImageOverlaySvg().replace(/^<svg[^>]*>/, '').replace(/<\/svg>$/, '')}
