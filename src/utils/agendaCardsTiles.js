...
export const AGENDA_CARDS_GEOM = {
  viewW: 1000,
  viewH: 560,
  n: 4,
  headingX: 48,
...
export const AGENDA_CARDS_PALETTE = ['#2F6FED', '#E85D4C', '#2A9D8F', '#E09F3E']
...
export function agendaCardsGraphicFrame(canvasW, canvasH) {
  return {
    graphicX: 0,
    graphicY: 0,
    graphicW: canvasW,
...
export function agendaCardsChromeSpecs() {
...
export function agendaCardsOverlayPlacements(gx, gy, gw, gh) {
  const g = AGENDA_CARDS_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToAgendaCardsContent(spec) {
  return { svg: cardInlineSvg(spec?.n || 1), colorMode: 'recolorable', fill: spec.color }
}

export function agendaCardsPreviewSvg() {
...
export function isAgendaCardsLayout(layoutId, family, variant) {
  if (family === 'minimal' && variant === 'cards') return true
  return /agenda_cards_v1$/i.test(String(layoutId || ''))
}

export function isAgendaCardsTextSlot(slotId) {
