...
export const PRICING_FPAC_GEOM = {
  viewW: 1000,
  viewH: 560,
  headingX: 40,
  headingY: 12,
...
export const PRICING_FPAC_PALETTE = ['#C4A35A', '#1B365D', '#1F7A5C', '#3A2F45']
...
export const PRICING_FPAC_DEFAULTS = {
...
export function isPricingFourParaCardsLayout(layoutId) {
  return /pricing_four_para_cards_v1$/i.test(String(layoutId || ''))
}

export function isPricingFourParaCardsTextSlot(slotId) {
...
export function pricingFourParaCardsChromeSpecs() {
  const g = PRICING_FPAC_GEOM
  const w = cardW()
  const specs = []
...
export function pricingFourParaCardsOverlay(gx, gy, gw, gh) {
  const g = PRICING_FPAC_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToPricingFourParaCardsContent(spec) {
  return { svg: cardChromeSvg(spec), colorMode: 'recolorable', fill: spec.color }
}

export function pricingFourParaCardsPreviewSvg() {
...
export function layoutPricingFourParaCards(elements, schema, palette = {}, canvas = {}) {
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    next.push(placeText(`PLAN_${n}_CTA`, overlay.ctas[i], {
      align: 'left', verticalAlign: 'center', fontSize: 12, fontWeight: 700, color: pal, clipToSlot: true, lineHeight: 1, fill: 'none', background: 'transparent',
    }, 'caption'))
  }
