...
export const TPH_PALETTE = ['#5B21B6', '#BE185D', '#DC2626', '#059669', '#EA580C', '#CA8A04']

const LOREM = 'Lorem ipsum dolor sit amet.'

export const TPH_DEFAULTS = {
...
export function timelineProcessHorizontalChromeSpecs() {
  const g = TPH_GEOM
  const specs = [{
    slotId: 'TPH_LINES',
    x: 0,
...
export function timelineProcessHorizontalOverlay(gx, gy, gw, gh) {
  const g = TPH_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineProcessHorizontalContent(spec) {
...
export function layoutTimelineProcessHorizontal(elements, schema, palette = {}, canvas = {}) {
  ...
  for (let i = 0; i < TPH_GEOM.n; i += 1) {
    ...
    next.push(placeText(`step_${n}_year`, overlay.years[i], {
      align: 'center', verticalAlign: 'center', fontSize: 20, fontWeight: 700, color: '#FFFFFF', clipToSlot: false, lineHeight: 1,
    }, 'caption'))
    
    next.push(placeText(`step_${n}_title`, overlay.titles[i], {
      align: 'center', verticalAlign: 'top', fontSize: 16, fontWeight: 700, color: color, clipToSlot: true, lineHeight: 1.2, wrap: 'wrap',
    }, 'heading'))
    
    next.push(placeText(`step_${n}_desc`, overlay.descs[i], {
      align: 'center', verticalAlign: 'top', fontSize: 11, fontWeight: 400, color: '#6B7280', clipToSlot: true, lineHeight: 1.3, wrap: 'wrap',
    }, 'body'))
  }
...
export function timelineProcessHorizontalPreviewSvg() {
  const specs = timelineProcessHorizontalChromeSpecs()
  const g = TPH_GEOM
  const parts = specs.map((spec) => {
    const inner = specToTimelineProcessHorizontalContent(spec).svg
...
export { isTimelineProcessHorizontalLayout, isTimelineProcessHorizontalTextSlot }
