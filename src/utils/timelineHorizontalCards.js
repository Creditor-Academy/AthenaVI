...
export const TLHC_PALETTE = ['#E4453A', '#E0892A', '#3D9A5F', '#3B82C4', '#7B5EA7']
...
export function isTimelineHorizontalCardsLayout(layoutId) {
  return /timeline_horizontal_cards_v1$/i.test(String(layoutId || ''))
}

export function isTimelineHorizontalCardsTextSlot(slotId) {
...
export function timelineHorizontalCardsChromeSpecs() {
  const g = TLHC_GEOM
  const w = cardW()
  const specs = [{
    slotId: 'TLHC_SPINE',
...
export function timelineHorizontalCardsOverlay(gx, gy, gw, gh) {
  const g = TLHC_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineHorizontalCardsContent(spec) {
...
export function layoutTimelineHorizontalCards(elements, schema, palette = {}, canvas = {}) {
