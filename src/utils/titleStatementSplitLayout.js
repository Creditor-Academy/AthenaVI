...
export function isTitleStatementSplitLayout(layoutId, schema) {
...
export const TITLE_STATEMENT_SPLIT_DEFAULTS = {
...
export function buildTitleStatementSplitChromeSvg() {
...
export function layoutTitleStatementSplit(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildTitleStatementSplitCanvasElements({ schema, options = {} } = {}) {
...
export function titleStatementSplitPreviewSvg() {
  const { headX, headY, headW, bodyX, bodyY, bodyW } = GEOM
  const chrome = buildTitleStatementSplitChromeSvg()
    .replace(/currentColor/g, '#6366F1')
    .replace(/^<svg[^>]*>/, '')
    .replace(/<\/svg>\s*$/, '')
  const headLines = ['Bold opening', 'statement']
  const bodyLines = [
    'Supporting copy that explains',
    'the idea in a few clear lines',
    'on the other side of the split.',
  ]
  const head = headLines
    .map((l, i) => `<text x="${headX}" y="${headY + 200 + i * 64}" fill="#0F172A" font-size="56" font-weight="800" font-family="system-ui, sans-serif">${l}</text>`)
    .join('')
  const body = bodyLines
    .map((l, i) => `<text x="${bodyX}" y="${bodyY + 160 + i * 48}" fill="#64748B" font-size="32" font-family="system-ui, sans-serif">${l}</text>`)
    .join('')
