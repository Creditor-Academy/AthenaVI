...
export const PRICING_TF_PALETTE = [
...
export const PRICING_TF_DEFAULTS = {
...
export function isPricingThreePlansFeaturedLayout(layoutId) {
  return /pricing_three_plans_featured_v1$/i.test(String(layoutId || ''))
}

export function isPricingThreePlansFeaturedTextSlot(slotId) {
...
export function pricingThreePlansFeaturedChromeSpecs() {
  const g = PRICING_TF_GEOM
  const w = cardW()
  const h = graphicH()
  const specs = []
...
export function pricingThreePlansFeaturedOverlay(gx, gy, gw, gh) {
  const g = PRICING_TF_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToPricingThreePlansFeaturedContent(spec) {
  return { svg: cardChromeSvg(spec), colorMode: 'recolorable', fill: spec.color }
}

export function pricingThreePlansFeaturedPreviewSvg() {
...
export function layoutPricingThreePlansFeatured(elements, schema, palette = {}, canvas = {}) {
