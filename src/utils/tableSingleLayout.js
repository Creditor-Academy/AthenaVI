/**
 * Table Single — Annual Financial Summary Table
...
export const TABLE_SINGLE_GEOM = {
  viewW: 1000,
  viewH: 560,
  headingX: 36,
  headingY: 16,
...
export const TABLE_SINGLE_PALETTE = {
  ribbon: '#123F67',
  ribbonFold: '#0A2640',
...
export const TABLE_SINGLE_DEFAULTS = {
...
export function isTableSingleLayout(layoutId) {
  const id = String(layoutId || '')
  if (/cards/i.test(id)) return false
  return /table_single_v1$/i.test(id)
}

export function isTableSingleTextSlot(slotId) {
...
export function tableSingleChromeSvg() {
  ...
  for (let r = 0; r < g.rows; r += 1) {
    ...
    if (r === g.rows - 1) fill = p.rowHighlight // Highlight for Net Income row
    rowRects.push(`<rect x="${g.tableX}" y="${y}" width="${g.tableW}" height="${g.rowH}" fill="${fill}" />`)
  }

  // 2. Vertical column dividers (between label and 5 data cols)
  const vDividers = []
  // Divider between label col and col 1
  ...
  for (let c = 0; c < g.cols; c += 1) {
    ...
    // Pentagon pointing downwards into the table column
  ...
  const ribbonSvg = `
    ...
    <!-- Dog-ear fold triangle on top-right -->
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${g.viewW} ${g.viewH}" width="100%" height="100%">
    ...
    <!-- Table Grid with Rounded Right Edge -->
    <g clip-path="url(#tableRoundedClip)">
      ${rowRects.join('\n')}
      ${vDividers.join('\n')}
      ${hDividers.join('\n')}
    </g>
    <!-- Table Outer Subtle Border -->
...
export function tableSingleChromeSpecs() {
  const g = TABLE_SINGLE_GEOM
  return [
    {
...
export function tableSingleOverlay(gx, gy, gw, gh) {
  const g = TABLE_SINGLE_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  ...
  // Column header badges (5 columns)
  const colHeaders = []
  const colSubs = []
  for (let c = 0; c < g.cols; c += 1) {
    const x = g.tableX + g.labelW + c * g.colW
...
export function specToTableSingleContent(spec) {
  return { svg: tableSingleChromeSvg(), colorMode: 'recolorable', fill: spec?.color || TABLE_SINGLE_PALETTE.ribbon }
}

export function tableSinglePreviewSvg() {
  return tableSingleChromeSvg()
}
...
export function layoutTableSingle(elements, schema, palette = {}, canvas = {}) {
