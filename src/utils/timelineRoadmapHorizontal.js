...
export function isTimelineRoadmapHorizontalLayout(layoutId) {
  return /timeline_roadmap_horizontal_v1$/i.test(String(layoutId || ''))
}

export function isTimelineRoadmapHorizontalTextSlot(slotId) {
...
export function timelineRoadmapHorizontalChromeSpecs() {
  const g = TLRH_GEOM
  const specs = [{
    slotId: 'TLRH_ROAD',
    x: 0,
...
export function timelineRoadmapHorizontalOverlay(gx, gy, gw, gh) {
  const g = TLRH_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineRoadmapHorizontalContent(spec) {
...
export function timelineRoadmapHorizontalPreviewSvg() {
  const specs = timelineRoadmapHorizontalChromeSpecs()
  const g = TLRH_GEOM
  const parts = specs.map((spec) => {
    const inner = specToTimelineRoadmapHorizontalContent(spec).svg
...
export function layoutTimelineRoadmapHorizontal(elements, schema, palette = {}, canvas = {}) {
