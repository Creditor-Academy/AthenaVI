...
export function buildHeadlineCenteredChromeSvg() {
...
export function layoutHeadlineCentered(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildHeadlineCenteredCanvasElements({ schema, options = {} } = {}) {
...
export function headlineCenteredPreviewSvg() {
  const { headX, headY, headW, bodyX, bodyY, bodyW, barY, barW, barH, viewW } = GEOM
  const barX = (viewW - barW) / 2
  const chrome = buildHeadlineCenteredChromeSvg()
    .replace(/currentColor/g, '#6366F1')
    .replace(/^<svg[^>]*>/, '')
    .replace(/<\/svg>\s*$/, '')
  const headLines = ['Opening headline']
  const bodyLines = [
    'Supporting copy that explains the idea',
    'in a few clear, centered lines.',
  ]
  const head = headLines
    .map((l, i) => `<text x="${headX + headW / 2}" y="${headY + 140 + i * 56}" text-anchor="middle" fill="#0F172A" font-size="56" font-weight="800" font-family="system-ui, sans-serif">${l}</text>`)
    .join('')
  const body = bodyLines
    .map((l, i) => `<text x="${bodyX + bodyW / 2}" y="${bodyY + 44 + i * 48}" text-anchor="middle" fill="#64748B" font-size="32" font-family="system-ui, sans-serif">${l}</text>`)
    .join('')
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    <rect width="1920" height="1080" fill="#FFFFFF" />
    ${chrome}
    ${head}
    <rect x="${barX}" y="${barY}" width="${barW}" height="${barH}" rx="${barH / 2}" fill="#6366F1" />
    ${body}
  </svg>`
}
