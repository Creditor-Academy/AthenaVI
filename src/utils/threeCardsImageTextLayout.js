...
export function isThreeCardsImageTextLayout(layoutId, schema) {
...
export const THREE_CARDS_IMAGE_TEXT_DEFAULTS = {
...
export function buildThreeCardsImageTextChromeSvg() {
  const {
...
export function layoutThreeCardsImageText(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildThreeCardsImageTextCanvasElements({ schema, options = {} } = {}) {
...
export function threeCardsImageTextPreviewSvg() {
  const {
    cardXs, cardY, cardW, imgPad, imgH, imgR, textPad, titleDy, bodyDy, headX, headY,
  } = GEOM
  const titles = THREE_CARDS_IMAGE_TEXT_DEFAULTS.TITLES
  const bodyLines = ['Supporting copy that', 'explains the key idea', 'in a few clear lines.']
  const photos = cardXs
    .map((x, i) => {
      const ix = x + imgPad
      ...
      const body = bodyLines
        ...
        .join('')
      return `
    <clipPath id="tciPrevClip${i}"><rect x="${ix}" y="${iy}" width="${iw}" height="${imgH}" rx="${imgR}" /></clipPath>
    <g clip-path="url(#tciPrevClip${i})">
      <rect x="${ix}" y="${iy}" width="${iw}" height="${imgH}" fill="url(#tciPrevSky)" />
      <ellipse cx="${ix + 130 + i * 60}" cy="${iy + 70}" rx="64" ry="26" fill="#FFFFFF" opacity="0.95" />
      <ellipse cx="${ix + 186 + i * 60}" cy="${iy + 70}" rx="44" ry="20" fill="#FFFFFF" opacity="0.95" />
      <path d="M${ix} ${iy + 170} C${ix + 110} ${iy + 125} ${ix + 220} ${iy + 180} ${ix + 330} ${iy + 155} C${ix + 400} ${iy + 140} ${ix + 450} ${iy + 165} ${ix + iw} ${iy + 150} L${ix + iw} ${iy + imgH} L${ix} ${iy + imgH} Z" fill="url(#tciPrevHill)" />
    </g>
    ...
    .join('')
  const chrome = buildThreeCardsImageTextChromeSvg()
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    ...
    <defs>
      <linearGradient id="tciPrevSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8" />
        <stop offset="100%" stop-color="#D0E3EF" />
      </linearGradient>
      <linearGradient id="tciPrevHill" x1="0" y1="0" x2="0" y2="1">
    ...
    ${chrome}
    ${photos}
    <text x="${headX}" y="${headY + 76}" fill="#0F172A" font-size="80" font-weight="800" font-family="system-ui, sans-serif">Product highlights</text>
  </svg>`
}
