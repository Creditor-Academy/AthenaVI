export const SWOT_N = 4
export const SWOT_LETTERS = ['S', 'W', 'O', 'T']
export const SWOT_COLORS = ['#7d9b6a', '#3d5a80', '#3aa8a4', '#d4785a']
export const SWOT_LABELS = ['STRENGTHS', 'WEAKNESSES', 'OPPORTUNITIES', 'THREATS']

export const SWOT_GEOM = {
  viewW: 980,
  viewH: 920,
  hubX: 268,
  hubY: 460,
...
export function swotPetalGeom(i) {
...
export function swotPetalViewBox(i) {
...
export function swotPetalPlacement(gx, gy, gw, gh, i) {
...
export function swotPetalInlineSvg(i) {
  const g = swotPetalGeom(i)
  const v = swotPetalViewBox(i)
...
export function swotIconInlineSvg(i) {
...
export function swotDashInlineSvg() {
...
export function swotGraphicBox(canvasW, canvasH) {
  const headingY = 32
  const headingH = 80
...
export function swotOverlayPlacements(frame) {
...
export function swotDiagramInlineSvg(colors = SWOT_COLORS) {
  const { viewW, viewH, hubX, hubY, hubR } = SWOT_GEOM
  const parts = []
  for (let i = 0; i < SWOT_N; i += 1) {
    const g = swotPetalGeom(i)
    parts.push(`<path d="${capsulePath(g.ax, g.ay, g.bx, g.by, g.r)}" fill="${colors[i]}"/>`)
    parts.push(`<text x="${g.lx.toFixed(1)}" y="${(g.ly + 12).toFixed(1)}" text-anchor="middle" fill="#fff" font-size="42" font-weight="800" font-family="system-ui,sans-serif">${SWOT_LETTERS[i]}</text>`)
  }
  parts.push(`<circle cx="${hubX}" cy="${hubY}" r="${hubR}" fill="#ffffff"/>`)
  parts.push(`<text x="${hubX}" y="${hubY - 8}" text-anchor="middle" fill="#111827" font-size="52" font-weight="800" font-family="system-ui,sans-serif">SWOT</text>`)
  parts.push(`<text x="${hubX}" y="${hubY + 36}" text-anchor="middle" fill="#6b7280" font-size="22" font-weight="600" font-family="system-ui,sans-serif">Analysis</text>`)
...
export function swotModeFromSchema(schema) {
...
export function swotQuadFrame(canvasW, canvasH, mode = 'grid') {
  const headingY = 56
  const headingH = 88
  const insetX = 64
