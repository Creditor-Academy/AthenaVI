...
export const PRICING_PCC_PALETTE = [
...
export const PRICING_PCC_DEFAULTS = {
...
export function isPricingComparisonCardsLayout(layoutId) {
  return /pricing_comparison_cards_v1$/i.test(String(layoutId || ''))
}

export function isPricingComparisonCardsTextSlot(slotId) {
...
export function pricingComparisonCardsChromeSpecs() {
  const g = PRICING_PCC_GEOM
  const w = cardW()
  const specs = []
...
export function pricingComparisonCardsOverlay(gx, gy, gw, gh) {
  const g = PRICING_PCC_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToPricingComparisonCardsContent(spec) {
  return { svg: cardChromeSvg(spec), colorMode: 'recolorable', fill: spec.color }
}

export function pricingComparisonCardsPreviewSvg() {
...
export function layoutPricingComparisonCards(elements, schema, palette = {}, canvas = {}) {
