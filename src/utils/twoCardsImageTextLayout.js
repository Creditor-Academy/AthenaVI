...
export function isTwoCardsImageTextLayout(layoutId, schema) {
...
export const TWO_CARDS_IMAGE_TEXT_DEFAULTS = {
...
export function buildTwoCardsImageTextChromeSvg() {
  const {
...
export function layoutTwoCardsImageText(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildTwoCardsImageTextCanvasElements({ schema, options = {} } = {}) {
...
export function twoCardsImageTextPreviewSvg() {
  const {
    cardXs, cardY, cardW, imgPad, imgH, imgR, textPad, titleDy, bodyDy, eyeX, eyeY,
  } = GEOM
  const titles = TWO_CARDS_IMAGE_TEXT_DEFAULTS.TITLES
  const bodyLines = [
    ['Lead with the first idea in a', 'few clear lines a viewer can', 'scan without crowding the photo.'],
    ['Pair it with a second idea that', 'complements the first, so the', 'two cards feel like a set.'],
  ]
  const photos = cardXs
    .map((x, i) => {
      const ix = x + imgPad
      const iy = cardY + imgPad
      const iw = cardW - imgPad * 2
      const body = bodyLines[i]
        .map((l, j) => `<text x="${x + textPad}" y="${cardY + bodyDy + 34 + j * 48}" fill="#64748B" font-size="32" font-family="system-ui, sans-serif">${l}</text>`)
        .join('')
      return `
    <clipPath id="tci2PrevClip${i}"><rect x="${ix}" y="${iy}" width="${iw}" height="${imgH}" rx="${imgR}" /></clipPath>
    <g clip-path="url(#tci2PrevClip${i})">
      <rect x="${ix}" y="${iy}" width="${iw}" height="${imgH}" fill="url(#tci2PrevSky)" />
      <ellipse cx="${ix + 180 + i * 80}" cy="${iy + 90}" rx="90" ry="36" fill="#FFFFFF" opacity="0.95" />
      <ellipse cx="${ix + 260 + i * 80}" cy="${iy + 90}" rx="60" ry="28" fill="#FFFFFF" opacity="0.95" />
      <path d="M${ix} ${iy + 250} C${ix + 180} ${iy + 180} ${ix + 360} ${iy + 270} ${ix + 540} ${iy + 220} C${ix + 680} ${iy + 190} ${ix + 740} ${iy + 240} ${ix + iw} ${iy + 210} L${ix + iw} ${iy + imgH} L${ix} ${iy + imgH} Z" fill="url(#tci2PrevHill)" />
    </g>
    <text x="${x + textPad}" y="${cardY + titleDy + 44}" fill="#0F172A" font-size="46" font-weight="800" font-family="system-ui, sans-serif">${titles[i]}</text>
    ${body}`
    })
    .join('')
  const chrome = buildTwoCardsImageTextChromeSvg()
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    ...
    <defs>
      <linearGradient id="tci2PrevSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8" />
        <stop offset="100%" stop-color="#D0E3EF" />
      </linearGradient>
      <linearGradient id="tci2PrevHill" x1="0" y1="0" x2="0" y2="1">
    ...
    ${chrome}
    ${photos}
    <text x="${eyeX}" y="${eyeY + 32}" fill="#6366F1" font-size="28" font-weight="700" font-family="system-ui, sans-serif" letter-spacing="1.2">Describe this slide</text>
  </svg>`
}
