...
export function buildTitleStatementChromeSvg() {
...
export function layoutTitleStatement(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildTitleStatementCanvasElements({ schema, options = {} } = {}) {
...
export function titleStatementPreviewSvg() {
  const { headX, headY, bodyX, bodyY } = GEOM
  const chrome = buildTitleStatementChromeSvg()
  ...
  const bodyLines = [
    ...
    'in a few clear lines under the headline.',
  ]
  const head = headLines
    .map((l, i) => `<text x="${headX}" y="${headY + 80 + i * 64}" fill="#0F172A" font-size="56" font-weight="800" font-family="system-ui, sans-serif">${l}</text>`)
    .join('')
  const body = bodyLines
    .map((l, i) => `<text x="${bodyX}" y="${bodyY + 40 + i * 48}" fill="#64748B" font-size="32" font-family="system-ui, sans-serif">${l}</text>`)
    .join('')
