...
export const PRICING_PCT_DEFAULTS = {
...
export function isPricingComparisonTableLayout(layoutId) {
  const id = String(layoutId || '')
  if (/cards/i.test(id)) return false
  return /pricing_comparison_table_v1$/i.test(id)
}

export function isPricingComparisonTableTextSlot(slotId) {
...
export function pricingComparisonTableChromeSpecs() {
  const g = PRICING_PCT_GEOM
  return [{
    slotId: 'PRICING_PCT',
    x: g.x,
...
export function pricingComparisonTableOverlay(gx, gy, gw, gh) {
  const g = PRICING_PCT_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToPricingComparisonTableContent(spec) {
  return { svg: tableChromeSvg(), colorMode: 'recolorable', fill: spec.color }
}

export function pricingComparisonTablePreviewSvg() {
...
export function layoutPricingComparisonTable(elements, schema, palette = {}, canvas = {}) {
