...
export const PRICING_FPA_PALETTE = ['#3D5A80', '#EE6C4D', '#2A9D8F', '#6D597A']
...
export const PRICING_FPA_DEFAULTS = {
...
export function isPricingFourParaLayout(layoutId) {
  const id = String(layoutId || '')
  if (/cards/i.test(id)) return false
  return /pricing_four_para_v1$/i.test(id)
}

export function isPricingFourParaTextSlot(slotId) {
...
export function pricingFourParaChromeSpecs() {
  const specs = []
...
export function pricingFourParaOverlay(gx, gy, gw, gh) {
  const g = PRICING_FPA_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToPricingFourParaContent(spec) {
  return { svg: cardChromeSvg(spec), colorMode: 'recolorable', fill: spec.color }
}

export function pricingFourParaPreviewSvg() {
...
export function layoutPricingFourPara(elements, schema, palette = {}, canvas = {}) {
