...
export const TLMIR_PALETTE = ['#1F4E79', '#1F8A7A', '#C45C26', '#6B2D5B']
...
export function isTimelineMilestonesImageRightLayout(layoutId) {
  return /timeline_milestones_image_right_v1$/i.test(String(layoutId || ''))
}

export function isTimelineMilestonesImageRightTextSlot(slotId) {
...
export function timelineMilestonesImageRightChromeSpecs() {
  const g = TLMIR_GEOM
  const specs = [{
    slotId: 'TLMIR_SPINE',
    x: 0,
...
export function timelineMilestonesImageRightOverlay(gx, gy, gw, gh) {
  const g = TLMIR_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineMilestonesImageRightContent(spec) {
...
export function layoutTimelineMilestonesImageRight(elements, schema, palette = {}, canvas = {}) {
