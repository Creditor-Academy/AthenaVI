/**
 * Process Linear Horizontal ("process_linear_horti") Layout
 ...
 *
 * Recolorability:
 *  - Each step badge is an individual graphic element: STEP_n_SHAPE with colorMode: 'recolorable'
 *  - Selecting any badge allows instant recoloring of its ring, connector, arc, and dot
 *  - User custom colors are persisted across layout updates
 */

export const PROCESS_LINEAR_HORTI_GEOM = {
  viewW: 1000,
  viewH: 560,
  ...
  // Text below
  titleOffsetY: 72, // cy + 72 = 300
  titleW: 172,
  titleH: 26,
  bodyOffsetY: 102, // cy + 102 = 330
  bodyW: 172,
  bodyH: 88,
}

// Default harmonious palette from the reference image:
// 1: Cyan/Teal, 2: Coral Red, 3: Slate Blue, 4: Golden Yellow, 5: Sage Green
export const PROCESS_LINEAR_HORTI_DEFAULT_COLORS = [
  '#54B2C8', // 1: Cyan / Teal
  '#E85B4E', // 2: Coral Red
  '#4E637D', // 3: Slate Blue / Navy
  '#EAB327', // 4: Warm Golden Yellow
  '#80B58E', // 5: Sage Green
  '#9333EA', // 6: Deep Purple (if 6 steps)
  '#EA580C', // 7: Vibrant Amber (if 7 steps)
]

export const PROCESS_LINEAR_HORTI_DEFAULT_STEPS = [
  {
    title: 'Discovery',
...
export function isProcessLinearHortiLayout(layoutId) {
...
// 5 Vector Icons matching reference image (handshake, briefcase, lightbulb, climbing stairs, award ribbon)
export const PROCESS_LINEAR_HORTI_ICON_PATHS = [
  // 1: Handshake
  `
 ...
 */
export function calculateProcessHortiStepGeometries(stepCount = 5) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    // Bounding box for the step badge element
    const x = cx - g.rOuter
...
/**
 * Builds standalone SVG for a single step badge (outer ring + inner white disc + connector + arc + dot).
 * Uses currentColor so AthenaVI's recolorable system updates the entire badge when selected!
 */
export function buildProcessHortiStepSvg(step) {
  const g = PROCESS_LINEAR_HORTI_GEOM
  const { localCx, localCy, w, h, isLast, arcDx, arcDy } = step

  // Connector line:
  // For non-last steps: extends to the next circle
  // For the last step: subtle lead-out that meets the arc start
 ...
 */
export function buildProcessHortiIconSvg(iconPaths) {
 ...
 */
export function processLinearHortiPreviewSvg(previewHints = {}, theme = {}) {
  const g = PROCESS_LINEAR_HORTI_GEOM
  const headingText = previewHints?.slots?.HEADING?.text || 'PROCESS FLOW INFOGRAPHICS'
  const subtitleText = previewHints?.slots?.SUBTITLE?.text || 'At vero eos et accusamus et iusto odio dignissimos ducimus qui blanditiis praesentium voluptatum deleniti atque corrupti'
  ...
  steps.forEach((step) => {
    ...
    const lineEndX = step.isLast ? step.cx + g.rArc : step.nextCx - g.rOuter

    svgs += `
      <!-- Connector line -->
      <line x1="${step.cx + g.rOuter}" y1="${step.cy}" x2="${lineEndX}" y2="${step.cy}" stroke="${color}" stroke-width="2.5" stroke-linecap="round" />
      
      <!-- Orbital arc -->
      <path d="M ${step.cx + g.rArc} ${step.cy} A ${g.rArc} ${g.rArc} 0 0 1 ${step.cx + step.arcDx} ${step.cy + step.arcDy}" fill="none" stroke="${color}" stroke-width="2.2" stroke-linecap="round" />
      
      <!-- Terminal dot -->
      <circle cx="${step.cx + step.arcDx}" cy="${step.cy + step.arcDy}" r="${g.dotRadius}" fill="${color}" />
      
      <!-- Outer thick colored ring -->
      <circle cx="${step.cx}" cy="${step.cy}" r="${g.rOuter}" fill="${color}" />
      
      <!-- Inner solid white disc -->
      <circle cx="${step.cx}" cy="${step.cy}" r="${g.rInner}" fill="#FFFFFF" />

      <!-- Center Icon -->
      <g transform="translate(${step.cx - g.iconSize / 2}, ${step.cy - g.iconSize / 2})" color="${color}" stroke="${color}" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ${step.iconPaths}
      </g>
    `
    ...
    textLabels += `
      <text x="${step.cx}" y="${titleY + 14}" text-anchor="middle" fill="${color}" font-size="13" font-weight="800" letter-spacing="0.5" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif">
        ${title.toUpperCase()}
      </text>
    `
    ...
    textLabels += `
      ...
      <text x="${step.cx}" y="${bodyY + 40}" text-anchor="middle" fill="#64748B" font-size="10.5" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif">
        ${line3}
      </text>
    `
  })

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${g.viewW} ${g.viewH}" width="100%" height="100%" style="background:#FAF8F5;">
    <!-- Title -->
    <text x="500" y="${g.headingY + 28}" text-anchor="middle" fill="#1E293B" font-size="26" font-weight="800" letter-spacing="1" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif">
      ${headingText}
    </text>
    <!-- Subtitle -->
    <text x="500" y="${g.subtitleY + 16}" text-anchor="middle" fill="#64748B" font-size="12" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif">
      ${subtitleText}
    </text>
    ${svgs}
    ${textLabels}
 ...
 *  - HEADING (text)
 *  - SUBTITLE (text)
 *  - STEP_n_SHAPE (graphic, recolorable badge with ring, connector, arc, and dot)
 *  - STEP_n_ICON (graphic, recolorable icon)
 *  - STEP_n_TITLE (text, colored with step accent)
 *  - STEP_n_BODY (text, body description)
 */
export function layoutProcessLinearHorti(elements = [], schema = {}, palette = {}, canvas = {}) {
  const g = PROCESS_LINEAR_HORTI_GEOM
  ...
  // 2. SUBTITLE Text Element (if slot exists or already present)
  ...
  // 3. Step Badges, Icons, Titles, and Descriptions
  steps.forEach((step) => {
    ...
    // 3A. Standalone Badge Graphic (ring, connector, orbital arc, and terminal dot)
    ...
    // 3B. Standalone Icon Graphic (centered inside the inner white circle)
