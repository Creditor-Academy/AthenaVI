...
export const TIMELINE_GEOM = { viewW: 1000, viewH: 120, nodeR: 22, spineH: 5 }

export const PROCESS_LINNER_GEOM = {
  viewW: 1000,
  viewH: 280,
  spineY: 36,
  nodeR: 14,
...
export const PROCESS_NUMERIC_GEOM = {
  viewW: 1000,
  viewH: 200,
  slotH: 4,
  badgeR: 36,
...
export const ROADMAP_GEOM = { viewW: 1000, viewH: 160, cardH: 100, nodeR: 18 }
...
export function horizontalTimelineInlineSvg(count = 4, { accent = '#6366f1', spine = '#1f2937', showChevrons = true } = {}) {
...
export function verticalTimelineInlineSvg(count = 3, { accent = '#6366f1', spine = '#94a3b8' } = {}) {
...
export function processLinnerHortiInlineSvg(count = 3, { accent = '#6366f1', phase = '#e8b4a0', spine = '#0f172a' } = {}) {
...
export function processLinnerNumericInlineSvg(count = 3, { colors = ['#a3e635', '#5eead4', '#fb7185', '#d4a574'] } = {}) {
  const n = Math.max(2, Math.min(4, Number(count) || 3))
...
export function roadmapTimelineInlineSvg(count = 4, { accent = '#6366f1', card = '#f1f5f9', spine = '#94a3b8' } = {}) {
...
export function milestonesImageTimelineInlineSvg(count = 3, { accent = '#6366f1', image = '#e2e8f0', spine = '#1f2937' } = {}) {
  const n = Math.max(2, Math.min(4, Number(count) || 3))
  const viewW = 1000
  const viewH = 260
  const pad = 48
...
export function timelineSpineSegmentInlineSvg() {
