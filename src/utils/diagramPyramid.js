export const PYRAMID_N = 5
export const PYRAMID_COLORS = ['#3b82f6', '#0f9d8a', '#86efac', '#f4c430', '#f97316']

export const PYRAMID_GEOM = {
  viewW: 900,
  viewH: 980,
  cx: 450,
  y0: 48,
...
export function pyramidModeFromSchema(schema) {
...
export function pyramidStageGeom(i, mode = 'classic') {
...
export function pyramidStageViewBox(i, mode = 'classic') {
  const g = pyramidStageGeom(i, mode)
...
export function pyramidStagePlacement(gx, gy, gw, gh, i, mode = 'classic') {
...
export function pyramidStageInlineSvg(i, mode = 'classic') {
  const v = pyramidStageViewBox(i, mode)
...
export function pyramidGraphicBox(canvasW, canvasH, mode = 'classic') {
...
export function pyramidLegendPlacements(frame, mode = 'classic') {
...
export function pyramidDiagramInlineSvg(colors = PYRAMID_COLORS, mode = 'classic') {
  const { viewW, viewH } = PYRAMID_GEOM
  const parts = []
  for (let i = 0; i < PYRAMID_N; i += 1) {
    const g = pyramidStageGeom(i, mode)
    parts.push(`<path d="${pyramidStagePath(g)}" fill="${colors[i]}"/>`)
    if (mode === 'layers') {
      const faces = pyramidLayerFaces(g)
      parts.push(`<path d="${faces.top}" fill="#ffffff" opacity="0.28"/>`)
      parts.push(`<path d="${faces.right}" fill="#000000" opacity="0.2"/>`)
    } else {
      parts.push(pyramidPeopleMarkup(g))
    }
  }
...
export const PYRAMID_BADGE_CLIP = 'polygon(0% 6%, 92% 50%, 0% 94%)'
