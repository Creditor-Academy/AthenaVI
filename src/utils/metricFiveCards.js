...
export const isMetricFiveCardsLayout = (layoutId) => {
  return /metric_five_cards_v1$/i.test(String(layoutId || ''))
}

export const isMetricFiveCardsTextSlot = (slotId) => {
...
export const metricFiveCardsChromeSpecs = () => {
  const g = M5C_GEOM
  const specs = []
...
export const metricFiveCardsOverlay = (gx, gy, gw, gh) => {
  const g = M5C_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToMetricFiveCardsContent = (spec) => {
...
export const layoutMetricFiveCards = (elements, schema, palette = {}, canvas = {}) => {
...
/**
 * Polished SVG Preview for Metric Five Cards thumbnail in slide picker.
 * Exact 1000x560 slide canvas matching rendered slide layout.
 */
export function metricFiveCardsPreviewSvg(previewHints = {}, theme = {}) {
