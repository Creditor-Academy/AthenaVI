...
export function chartTwoMetricsComparisonChromeSpecs() {
  const g = CTMC_GEOM
...
export function chartTwoMetricsComparisonOverlay(gx, gy, gw, gh) {
  const g = CTMC_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToChartTwoMetricsComparisonContent(spec) {
...
export function layoutChartTwoMetricsComparison(elements, schema, palette = {}, canvas = {}) {
  ...
  const filtered = elements.filter((el) => !chromeRe.test(String(el.slotId || '')) && isChartTwoMetricsComparisonTextSlot(el.slotId))
  ...
  const chrome = chartTwoMetricsComparisonChromeSpecs().map((spec) => {
    const prev = prevBySlot.get(spec.slotId.toUpperCase())
    const graphic = specToChartTwoMetricsComparisonContent(spec)
    if (!graphic) return null
    return {
      id: prev?.id || newId('shp-ctmc'),
      type: 'graphic',
      layer: spec.layer || 4,
      placement: {
...
export { isChartTwoMetricsComparisonLayout, isChartTwoMetricsComparisonTextSlot }
