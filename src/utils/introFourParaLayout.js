 ...
 *
 * Intro heading plus a 2×2 grid of numbered paragraph cards.
 * Distinct from Intro Three Para Icons and Four Para Image.
 */

export function isIntroFourParaLayout(layoutId, schema) {
  const id = String(layoutId || schema?.layout_id || schema?.variant || '').toLowerCase()
  return id === 'intro_four_para_v1' || id === 'intro_four_para'
}

export const INTRO_FOUR_PARA_DEFAULTS = {
  INTRO: 'What we will cover',
  ITEM_1: 'Introduction — context, audience, and the outcome this story is here to drive.',
  ITEM_2: 'Problem and opportunity — where the current path breaks, and the opening it creates.',
  ITEM_3: 'Solution and proof — the approach, evidence, and why it holds up under scrutiny.',
  ITEM_4: 'Next steps — owners, timing, and the first decisions that move the work forward.',
}
...
export function buildIntroFourParaChromeSvg() {
...
export function layoutIntroFourPara(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildIntroFourParaCanvasElements({ schema, options = {} } = {}) {
...
export function introFourParaPreviewSvg() {
  const chrome = buildIntroFourParaChromeSvg()
    .replace(/currentColor/g, '#6366F1')
    .replace(/^<svg[^>]*>/, '')
    .replace(/<\/svg>\s*$/, '')
  const intro = `<text x="${GEOM.introX}" y="${GEOM.introY + 72}" fill="#0F172A" font-size="48" font-weight="800" font-family="system-ui, sans-serif">What we will cover</text>`
  const paras = cardRects().map(({ x, y }, i) => {
    const labels = [
      'Introduction and the outcome this story drives.',
      'Problem, opportunity, and the opening it creates.',
      'Solution, proof, and why the approach holds up.',
      'Next steps, owners, and the first decisions.',
    ]
    return `<text x="${x + GEOM.cardPad}" y="${y + GEOM.textY + 36}" fill="#64748B" font-size="26" font-family="system-ui, sans-serif">${labels[i]}</text>`
  }).join('')
