/**
 * Table Single Cards — Investment Report Table
...
export const TABLE_SINGLE_CARDS_GEOM = {
  viewW: 1000,
  viewH: 560,
  headingX: 40,
  headingY: 16,
...
export const TABLE_SINGLE_CARDS_PALETTE = {
  headers: [
    '#43B5C0', // Col 0: Teal
    '#008CA8', // Col 1: Ocean Teal
    '#0073A8', // Col 2: Cerulean
    '#1C5685', // Col 3: Steel Blue
    '#124372', // Col 4: Navy Blue
    '#06255B', // Col 5: Midnight Navy
    '#104F8A', // Col 6: Deep Royal Navy
  ],
  col0Body: '#EEF2F6',
  cellNormal: '#F8FAFC',
  cellHighlight: '#CBD5E1', // Cols 3 and 5 highlighted as in reference
  totalCol0: '#0F4C81',
  totalCols: '#14558F',
  textDark: '#1E293B',
  textMuted: '#64748B',
...
export const TABLE_SINGLE_CARDS_DEFAULTS = {
...
export function isTableSingleCardsLayout(layoutId) {
  const id = String(layoutId || '')
  return /table_single_cards_v1$/i.test(id)
}

export function isTableSingleCardsTextSlot(slotId) {
...
/** Generates the SVG Chrome for Table Single Cards */
export function tableSingleCardsChromeSvg() {
  const g = TABLE_SINGLE_CARDS_GEOM
  const p = TABLE_SINGLE_CARDS_PALETTE

  // 1. Accent pill below subtitle
  const pillSvg = `
  ...
  // 2. Column Header Cards with rounded top corners
  const headerRects = []
  ...
  // 3. Body Cell Cards (4 rows x 7 cols)
  const bodyCellRects = []
...
export function tableSingleCardsChromeSpecs() {
  const g = TABLE_SINGLE_CARDS_GEOM
  return [
    {
...
export function tableSingleCardsOverlay(gx, gy, gw, gh) {
  const g = TABLE_SINGLE_CARDS_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  ...
  // Total row slots (Col 0 label + 6 total values)
  const totY = totalRowY()
...
export function specToTableSingleCardsContent(spec) {
  return { svg: tableSingleCardsChromeSvg(), colorMode: 'recolorable', fill: spec?.color || TABLE_SINGLE_CARDS_PALETTE.totalCol0 }
}

export function tableSingleCardsPreviewSvg() {
  return tableSingleCardsChromeSvg()
}
...
export function layoutTableSingleCards(elements, schema, palette = {}, canvas = {}) {
  ...
  // 4. 24 Body Cells (4 rows x 6 data cols)
  for (let r = 0; r < TABLE_SINGLE_CARDS_GEOM.bodyRows; r += 1) {
    const rowNum = r + 1
