...
export const isGridImagesTextMosaicLayout = (layoutId) => {
  return /grid_images_text_mosaic_v1$/i.test(String(layoutId || ''))
}

export const isGridImagesTextMosaicSlot = (slotId) => {
...
export const gridImagesTextMosaicChromeSpecs = () => {
  const g = GITM_GEOM
  const specs = []
...
export const gridImagesTextMosaicOverlay = (gx, gy, gw, gh) => {
  const g = GITM_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToGridImagesTextMosaicContent = (spec) => {
  if (spec.kind === 'card') {
    return {
...
export const layoutGridImagesTextMosaic = (elements, schema, palette = {}, canvas = {}) => {
