/** Left copy + stacked tablet / laptop / phone / watch cluster. */

export function isDeviceMultiClusterLayout(layoutId) {
...
function clusterGeom(canvasW, canvasH) {
  const padX = 96
  const copyW = 540
  const headingY = 200
  const headingH = 210
  ...
  return {
    ...
    bodyY: 528,
    bodyH: 220,
    glow: { x: 1080, y: 200, w: 620, h: 620, layer: 2 },
    devices: [
...
export function layoutDeviceMultiCluster(elements, schema, palette = {}, canvas = {}) {
  ...
  const g = clusterGeom(canvasW, canvasH)
  ...
  const next = stripped.map((el) => {
    ...
    const key = matchDevice(sid, role)
