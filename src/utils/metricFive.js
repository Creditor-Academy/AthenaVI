...
export const isMetricFiveLayout = (layoutId) => {
  return /metric_five_v1$/i.test(String(layoutId || ''))
}

export const isMetricFiveTextSlot = (slotId) => {
...
export const metricFiveChromeSpecs = () => {
  const g = MFIVE_GEOM
  const specs = []
...
export const metricFiveOverlay = (gx, gy, gw, gh) => {
  const g = MFIVE_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToMetricFiveContent = (spec) => {
...
export const layoutMetricFive = (elements, schema, palette = {}, canvas = {}) => {
...
/**
 * Polished SVG Preview for Metric Five thumbnail in slide picker.
 * Exact 1000x560 slide canvas matching rendered slide layout.
 */
export function metricFivePreviewSvg(previewHints = {}, theme = {}) {
