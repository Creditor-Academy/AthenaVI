/**
 * Two Para Right Image
 * Layout ID: two_para_right_image_v1
 *
 * Two stacked paragraphs on the left, rounded photo on the right.
 * Bottom twin keeps its own engine.
 */

export function isTwoParaRightImageLayout(layoutId, schema) {
...
export const TWO_PARA_RIGHT_IMAGE_DEFAULTS = {
...
export function buildTwoParaRightImageChromeSvg() {
...
export function layoutTwoParaRightImage(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildTwoParaRightImageCanvasElements({ schema, options = {} } = {}) {
  ...
  return buildElements({
    canvasW,
    canvasH,
    body1Text: String(contentBySlotId.BODY_1 || content.body || content.title || TWO_PARA_RIGHT_IMAGE_DEFAULTS.BODY_1).trim(),
    body2Text: String(contentBySlotId.BODY_2 || content.body2 || TWO_PARA_RIGHT_IMAGE_DEFAULTS.BODY_2).trim(),
    imageUrl:
...
export function twoParaRightImagePreviewSvg() {
  const { imgX, imgY, imgW, imgH, radius, pad, textX, barY, barW, barH } = GEOM
  const frameX = imgX - pad
  const frameY = imgY - pad
  const frameW = imgW + pad * 2
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    ...
    <defs>
      <filter id="tpriPrevShadow" x="-10%" y="-8%" width="120%" height="124%">
        <feDropShadow dx="0" dy="16" stdDeviation="18" flood-color="#94A3B8" flood-opacity="0.18" />
      </filter>
      <clipPath id="tpriClip"><rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" rx="${radius}" /></clipPath>
      <linearGradient id="tpriSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8" />
        <stop offset="55%" stop-color="#B7D4E8" />
        <stop offset="100%" stop-color="#D0E3EF" />
      </linearGradient>
      <linearGradient id="tpriHill" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#8FBF8E" />
        <stop offset="100%" stop-color="#5E945D" />
      </linearGradient>
    </defs>
    <ellipse cx="80" cy="60" rx="220" ry="140" fill="#6366F1" opacity="0.08" />
    <ellipse cx="1840" cy="1040" rx="260" ry="160" fill="#6366F1" opacity="0.06" />
    <rect x="${frameX}" y="${frameY}" width="${frameW}" height="${frameH}" rx="${frameR}" fill="#FFFFFF" filter="url(#tpriPrevShadow)" />
    <g clip-path="url(#tpriClip)">
      <rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" fill="url(#tpriSky)" />
      ...
      <path d="M${imgX} ${imgY + 420} C${imgX + 180} ${imgY + 320} ${imgX + 360} ${imgY + 400} ${imgX + 540} ${imgY + 360} C${imgX + 660} ${imgY + 330} ${imgX + 720} ${imgY + 400} ${imgX + imgW} ${imgY + 340} L${imgX + imgW} ${imgY + imgH} L${imgX} ${imgY + imgH} Z" fill="url(#tpriHill)" />
    </g>
    <rect x="${textX}" y="${barY}" width="${barW}" height="${barH}" rx="${barH / 2}" fill="#6366F1" />
    <text x="${textX}" y="304" fill="#0F172A" font-size="26" font-weight="700" font-family="system-ui, sans-serif">We help teams turn complex ideas into</text>
    <text x="${textX}" y="342" fill="#0F172A" font-size="26" font-weight="700" font-family="system-ui, sans-serif">clear narratives that drive decisions and</text>
    <text x="${textX}" y="380" fill="#0F172A" font-size="26" font-weight="700" font-family="system-ui, sans-serif">build momentum across the</text>
    <text x="${textX}" y="418" fill="#0F172A" font-size="26" font-weight="700" font-family="system-ui, sans-serif">organization.</text>
    <text x="${textX}" y="596" fill="#64748B" font-size="20" font-family="system-ui, sans-serif">Our approach combines research,</text>
    <text x="${textX}" y="628" fill="#64748B" font-size="20" font-family="system-ui, sans-serif">design, and storytelling so every slide</text>
    <text x="${textX}" y="660" fill="#64748B" font-size="20" font-family="system-ui, sans-serif">earns attention and every message lands</text>
    <text x="${textX}" y="692" fill="#64748B" font-size="20" font-family="system-ui, sans-serif">with precision.</text>
  </svg>`
}
