...
export const isGridMetricsMobileLayout = (layoutId) => {
  return /grid_metrics_mobile_v1$/i.test(String(layoutId || ''))
}

export const isGridMetricsMobileSlot = (slotId) => {
...
export const gridMetricsMobileChromeSpecs = () => {
  const g = GMM_GEOM
  const specs = []
...
export const gridMetricsMobileOverlay = (gx, gy, gw, gh) => {
  const g = GMM_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToGridMetricsMobileContent = (spec) => {
...
export const layoutGridMetricsMobile = (elements, schema, palette = {}, canvas = {}) => {
