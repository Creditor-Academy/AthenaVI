/** Centered phone with 3+3 feature callouts, icons, and connector lines. */
...
export const HIGHLIGHT_ICON_COLORS = ['#22c55e', '#1e3a8a', '#ef4444', '#38bdf8', '#f97316', '#64748b']
...
export function highlightIconSvg(index) {
  return ICON_SVGS[index % ICON_SVGS.length]
}

export function isDevicePhoneHighlightsLayout(layoutId) {
 ...
 * @returns {object[]}
 */
export function layoutDevicePhoneHighlights(elements, schema, palette = {}, canvas = {}) {
  ...
  HIGHLIGHT_FEATURES.forEach((feat, i) => {
    ...
    chrome.push({
      ...
      placement: {
        x: Math.round(lineX),
        y: Math.round(cy - 1),
        width: Math.max(8, Math.round(lineW)),
        height: 2,
        rotation: 0,
        opacity: 1,
      },
