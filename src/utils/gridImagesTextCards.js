...
export const isGridImagesTextCardsLayout = (layoutId) => {
  return /grid_images_text_cards_v1$/i.test(String(layoutId || ''))
}

export const isGridImagesTextCardsSlot = (slotId) => {
...
export const gridImagesTextCardsChromeSpecs = () => {
  const g = GITC_GEOM
  const specs = []
...
export const gridImagesTextCardsOverlay = (gx, gy, gw, gh) => {
  const g = GITC_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToGridImagesTextCardsContent = (spec) => {
  if (spec.kind === 'card') {
    return {
...
export const layoutGridImagesTextCards = (elements, schema, palette = {}, canvas = {}) => {
