/**
 * Para Title Left Image Boxed
 * Layout ID: para_title_left_image_boxed_v1
 *
 * Square rounded photo with a soft halo, accent dash, heading, body.
 */

export function isParaTitleLeftImageBoxedLayout(layoutId, schema) {
...
export const PARA_TITLE_LEFT_IMAGE_BOXED_DEFAULTS = {
...
export function buildParaTitleLeftImageBoxedChromeSvg() {
...
export function layoutParaTitleLeftImageBoxed(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildParaTitleLeftImageBoxedCanvasElements({ schema, options = {} } = {}) {
  ...
  return buildElements({
    canvasW,
    canvasH,
    headingText: String(contentBySlotId.HEADING || content.title || PARA_TITLE_LEFT_IMAGE_BOXED_DEFAULTS.HEADING).trim(),
    bodyText: String(contentBySlotId.BODY || content.body || PARA_TITLE_LEFT_IMAGE_BOXED_DEFAULTS.BODY).trim(),
    imageUrl:
...
export function paraTitleLeftImageBoxedPreviewSvg() {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    ...
    <defs>
      <filter id="ptlibPrevShadow" x="-12%" y="-10%" width="124%" height="128%">
        <feDropShadow dx="0" dy="18" stdDeviation="22" flood-color="#94A3B8" flood-opacity="0.22" />
      </filter>
      <clipPath id="ptlibClip"><rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" rx="${radius}" /></clipPath>
      <linearGradient id="ptlibSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8" />
        <stop offset="55%" stop-color="#B7D4E8" />
        <stop offset="100%" stop-color="#D0E3EF" />
      </linearGradient>
      <linearGradient id="ptlibHill" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#8FBF8E" />
        <stop offset="100%" stop-color="#5E945D" />
      </linearGradient>
    </defs>
    <rect x="${frameX}" y="${frameY}" width="${frameW}" height="${frameH}" rx="${frameR}" fill="#FFFFFF" filter="url(#ptlibPrevShadow)" />
    <g clip-path="url(#ptlibClip)">
      <rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" fill="url(#ptlibSky)" />
      ...
      <path d="M${imgX} ${imgY + 390} C${imgX + 160} ${imgY + 300} ${imgX + 300} ${imgY + 360} ${imgX + 440} ${imgY + 340} C${imgX + 560} ${imgY + 324} ${imgX + 640} ${imgY + 380} ${imgX + imgW} ${imgY + 330} L${imgX + imgW} ${imgY + imgH} L${imgX} ${imgY + imgH} Z" fill="url(#ptlibHill)" />
    </g>
