...
export const AGENDA_HERO_CARDS_GEOM = {
  viewW: 1000,
  viewH: 560,
  headingY: 228,
  headingH: 42,
...
export const HERO_CARD_PALETTE = [
...
export function agendaHeroCardsGraphicFrame(canvasW, canvasH) {
  return {
    graphicX: 0,
    graphicY: 0,
    graphicW: canvasW,
...
export function agendaHeroCardsCardInlineSvg(spec) {
  const w = spec.w
  const h = spec.h
...
export function agendaHeroCardsChromeSpecs() {
...
export function agendaHeroCardsOverlayPlacements(gx, gy, gw, gh) {
...
export function specToHeroCardsContent(spec) {
  return { svg: agendaHeroCardsCardInlineSvg(spec), colorMode: 'original', fill: spec.color }
}

export function isAgendaHeroCardsLayout(layoutId, family, variant) {
