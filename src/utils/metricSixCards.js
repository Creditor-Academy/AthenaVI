...
export const isMetricSixCardsLayout = (layoutId, schema = null) => {
...
export const isMetricSixCardsTextSlot = (slotId) => {
...
export const metricSixCardsChromeSpecs = () => {
  const g = M6C_GEOM
  const specs = []
...
export const metricSixCardsOverlay = (gx, gy, gw, gh) => {
  const g = M6C_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToMetricSixCardsContent = (spec) => {
...
export const layoutMetricSixCards = (elements, schema, palette = {}, canvas = {}) => {
 ...
 */
export function metricSixCardsPreviewSvg(previewHints = {}, theme = {}) {
