...
export const CT3_GEOM = {
  ...
  chartW: 600,
  chartH: 360,
  groupCount: 4,  // 4 groups
  barPadding: 6,  // Space between bars in a group
  groupPadding: 30,  // Space between groups
  // Legend (vertical, right side)
  legendX: 740,
  legendY: 140,
  legendBoxSize: 40,  // Larger colored boxes
  legendItemH: 100,   // Height per legend item (box + 2 lines of text)
  legendBoxW: 30,
...
export const CT3_PALETTE = {
  series1: '#FF6B35',  // Orange
  series2: '#FFC107',  // Yellow/Gold
  series3: '#4CAF50',  // Green
}

const LOREM = 'This is a sample text. Insert your desired text here.'

export const CT3_DEFAULTS = {
...
export function chartThreeBarChromeSpecs(chartData = {}) {
  const g = CT3_GEOM
  // Default data: 4 groups, 3 series each
  ...
  const max = Math.max(...series1Values, ...series2Values, ...series3Values, 5)
  ...
  const specs = [
    ...
    // Legend boxes (vertical on right)
    {
...
export function chartThreeBarOverlay(gx, gy, gw, gh) {
  const g = CT3_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToChartThreeBarContent(spec) {
...
export function layoutChartThreeBar(elements, schema, palette = {}, canvas = {}, chartData = {}) {
  ...
  const filtered = elements.filter((el) => !chromeRe.test(String(el.slotId || '')) && isChartThreeBarTextSlot(el.slotId))
  ...
  const series1 = chart1El?.content?.values || chart1El?.content?.data
  const series2 = chart2El?.content?.values || chart2El?.content?.data
  const series3 = chart3El?.content?.values || chart3El?.content?.data
  ...
  const chrome = chartThreeBarChromeSpecs(extractedData).map((spec) => {
    const prev = prevBySlot.get(spec.slotId.toUpperCase())
    const graphic = specToChartThreeBarContent(spec)
    return {
      id: prev?.id || newId('shp-ct3'),
      type: 'graphic',
      layer: spec.layer || 4,
      placement: {
...
export function chartThreeBarPreviewSvg() {
  const specs = chartThreeBarChromeSpecs()
  const g = CT3_GEOM
  const parts = specs.map((spec) => {
    const inner = specToChartThreeBarContent(spec).svg
...
export { isChartThreeBarLayout, isChartThreeBarTextSlot }
