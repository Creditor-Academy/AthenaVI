...
export const isGridThreeImagesTextAsymmetricLayout = (layoutId) => {
  return /grid_three_images_text_asymmetric_v1$/i.test(String(layoutId || ''))
}

export const isGridThreeImagesTextAsymmetricSlot = (slotId) => {
...
export const gridThreeImagesTextAsymmetricChromeSpecs = () => {
  const g = GTITA_GEOM
  const specs = []
...
export const gridThreeImagesTextAsymmetricOverlay = (gx, gy, gw, gh) => {
  const g = GTITA_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToGridThreeImagesTextAsymmetricContent = (spec) => {
...
export const layoutGridThreeImagesTextAsymmetric = (elements, schema, palette = {}, canvas = {}) => {
