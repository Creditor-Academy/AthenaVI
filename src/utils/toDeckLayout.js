import { DECK_LAYOUT_CATEGORIES } from './deckLayoutMeta.constants.js'
import { applyLayoutMetadataOverride } from './layoutMetadataOverrides.js'
...
function deriveContentCapacity(slots, layoutId, supported) {
  ...
  if (visibleSlots.length >= 8 || maxBullets >= 8 || cardCount >= 4 || /dense|compact|three_para|four_para/.test(layoutId)) {
    density = 'high';
  }
  ...
  return {
    ...
    maxSubtitleCharacters: supported.subtitle
      ...
      : 0,
    maxBodyCharacters: supported.body ? maxBodyCharacters : 0,
    maxBullets,
    ...
    maxColumns: Math.max(columnCount, imageCount && bodySlots.length ? 2 : 1),
    density,
  };
}
