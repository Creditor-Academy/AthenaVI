...
export const AGENDA_MINIMAL_QUIET_GEOM = {
  viewW: 1000,
  viewH: 560,
  headingX: 88,
  headingY: 52,
...
export function agendaMinimalQuietGraphicFrame(canvasW, canvasH) {
  return {
    graphicX: 0,
    graphicY: 0,
    graphicW: canvasW,
...
export function agendaMinimalQuietChromeSpecs() {
...
export function agendaMinimalQuietOverlayPlacements(gx, gy, gw, gh) {
  const g = AGENDA_MINIMAL_QUIET_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToMinimalQuietContent(spec) {
...
export function agendaMinimalQuietPreviewSvg() {
...
export function isAgendaMinimalQuietLayout(layoutId, family, variant) {
...
export function isAgendaMinimalQuietTextSlot(slotId) {
