/** Normalize chart payloads from AI / backend into renderer-friendly shape. */
...
export function extractChartSeriesValues(chartData = {}) {
  ...
  if (Array.isArray(series) && series[0] && typeof series[0] === 'object') {
    const fromObjects = series.map((row) => Number(row.value ?? row.y ?? row.v)).filter((v) => !Number.isNaN(v))
...
export function normalizeChartType(chartType) {
...
export function buildPremiumChartColors(palette = {}, count = 5) {
...
export function normalizeChartContent(raw = {}, palette = {}) {
  const labels = Array.isArray(raw.labels) ? raw.labels : Array.isArray(raw.data?.labels) ? raw.data.labels : []
