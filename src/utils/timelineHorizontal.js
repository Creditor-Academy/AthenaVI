...
export const TLH_PALETTE = ['#F05A4A', '#E7A033', '#74A572', '#44B1D2', '#27406A']

const LOREM = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore.'

export const TLH_DEFAULTS = {
...
export function isTimelineHorizontalLayout(layoutId) {
  const id = String(layoutId || '')
  if (/cards/i.test(id)) return false
  return /timeline_horizontal_v1$/i.test(id)
}

export function isTimelineHorizontalTextSlot(slotId) {
...
export function timelineHorizontalChromeSpecs() {
  const g = TLH_GEOM
  const w = cardW()
  const specs = [{
    slotId: 'TLH_SPINE',
...
export function timelineHorizontalOverlay(gx, gy, gw, gh) {
  const g = TLH_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineHorizontalContent(spec) {
...
export function timelineHorizontalPreviewSvg() {
...
export function layoutTimelineHorizontal(elements, schema, palette = {}, canvas = {}) {
