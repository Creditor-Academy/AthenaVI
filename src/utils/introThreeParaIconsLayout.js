/**
 * Intro Three Para Icons
 * Layout IDs:
 *  - intro_three_para_icons_v1: three stacked rows
 *  - intro_three_para_icons_horizontal_v1: three columns
 *
 * Recolorable icon chrome + numbered copy. Same slots as catalog.
 */
...
export const INTRO_THREE_PARA_ICONS_PALETTE = {
  primary: '#148A80',
  textDark: '#111827',
  textMuted: '#6B7280',
}
...
export function layoutIntroThreeParaIcons(docOrElements = [], schema = {}, palette = {}, canvas = {}) {
...
export function introThreeParaIconsPreviewSvg(options = {}) {
  ...
  if (isHorizontal) {
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      cardsSvg += `
        <g transform="translate(${cardX}, ${g.cardY})">
          <rect width="${g.cardW}" height="${g.cardH}" fill="#FBFBFA" />
          <rect width="${g.cardW}" height="3" fill="${accent}" />
          <circle cx="40" cy="48" r="22" fill="none" stroke="${accent}" stroke-width="1.4" />
          <g transform="translate(26, 34)">${renderPillarVectorIcon(theme.iconKind, accent)}</g>
          <text x="24" y="96" fill="${accent}" font-size="22" font-weight="800" font-family="Inter, sans-serif">${theme.index}</text>
          <rect x="24" y="132" width="180" height="16" rx="3" fill="#111827" />
          <rect x="24" y="184" width="248" height="8" rx="2" fill="#9CA3AF" />
          <rect x="24" y="202" width="248" height="8" rx="2" fill="#9CA3AF" />
          <rect x="24" y="220" width="200" height="8" rx="2" fill="#9CA3AF" />
        </g>`
    }
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 560" width="${width}" height="${height}">
      ...
      <text x="${g.badgeX}" y="${g.badgeY + 14}" fill="${accent}" font-size="11" font-weight="700" font-family="Inter, sans-serif" letter-spacing="0.16em">STRATEGIC FOUNDATION</text>
      <text x="${g.headingX}" y="${g.headingY + 26}" fill="#111827" font-size="28" font-weight="800" font-family="Inter, sans-serif">Three pillars</text>
      <text x="${g.subtitleX}" y="${g.subtitleY + 16}" fill="#6B7280" font-size="13" font-family="Inter, sans-serif">Core principles driving long-term strategic execution and organizational alignment.</text>
      ${cardsSvg}
    </svg>`
  }
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    const cy = g.cardH / 2
    rowsSvg += `
      <g transform="translate(${g.cardX}, ${cardY})">
        <rect width="${g.cardW}" height="${g.cardH}" fill="#FBFBFA" />
        <rect x="0" y="18" width="3" height="${g.cardH - 36}" rx="1.5" fill="${accent}" />
        <circle cx="64" cy="${cy}" r="26" fill="none" stroke="${accent}" stroke-width="1.4" />
        <g transform="translate(50, ${cy - 14})">${renderPillarVectorIcon(theme.iconKind, accent)}</g>
        <text x="108" y="36" fill="${accent}" font-size="13" font-weight="800" font-family="Inter, sans-serif">${theme.index}</text>
        <rect x="148" y="28" width="220" height="16" rx="3" fill="#111827" />
        <rect x="148" y="62" width="700" height="8" rx="2" fill="#9CA3AF" />
        <rect x="148" y="80" width="560" height="8" rx="2" fill="#9CA3AF" />
      </g>`
  }
