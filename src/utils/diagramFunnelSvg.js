...
export const FUNNEL_STAGE_COLORS = ['#1e3a5f', '#2563eb', '#ea580c', '#64748b']
export const FUNNEL_TITLE_COLORS = ['#1e3a5f', '#1d4ed8', '#c2410c', '#334155']

export const FUNNEL_GEOM = {
  viewW: 820,
  viewH: 980,
  cx: 310,
  y0: 36,
...
export function funnelStageGeom(i) {
...
export function funnelOverlayPlacements(gx, gy, gw, gh) {
...
export function estimateFunnelBodyHeight(text, width, fontSize = 16, lineHeight = 1.4, minLines = 1, maxLines = 5) {
 ...
 */
export function packFunnelStageTextBlocks({
  stages,
  bodies = [],
  textWidth = 400,
  titleH = 34,
...
export function funnelStageViewBox(i) {
  const g = funnelStageGeom(i)
  const yb = g.y + g.bandH
...
export function funnelStagePlacement(gx, gy, gw, gh, i) {
  const sx = gw / FUNNEL_GEOM.viewW
  const sy = gh / FUNNEL_GEOM.viewH
...
export function funnelStageInlineSvg(i) {
  const v = funnelStageViewBox(i)
...
export function buildFunnelDiagramSvg(colors = FUNNEL_STAGE_COLORS) {
...
export function funnelDiagramInlineSvg(colors = FUNNEL_STAGE_COLORS) {
...
export const FUNNEL_H_STAGE_COLORS = FUNNEL_STAGE_COLORS
export const FUNNEL_H_TITLE_COLORS = FUNNEL_TITLE_COLORS

export const FUNNEL_H_GEOM = {
  viewW: 1680,
  viewH: 700,
  cy: 292,
...
export function funnelHStageGeom(i) {
...
export function funnelHSegInlineSvg(i) {
...
export function funnelHSegPlacement(gx, gy, gw, gh, i) {
...
export function funnelHOverlayPlacements(gx, gy, gw, gh) {
  const sx = gw / FUNNEL_H_GEOM.viewW
  const sy = gh / FUNNEL_H_GEOM.viewH
  return {
...
export function funnelHColumnPlacements(gx, gy, gw, gh) {
  return funnelHOverlayPlacements(gx, gy, gw, gh).stages.map((s) => ({ x: s.x, w: s.w }))
}

export function funnelHPreviewSvg(colors = FUNNEL_STAGE_COLORS) {
