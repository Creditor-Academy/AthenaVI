...
export const isGridMetricsMasonryLayout = (layoutId) => {
  return /grid_metrics_masonry_v1$/i.test(String(layoutId || ''))
}

export const isGridMetricsMasonrySlot = (slotId) => {
...
export const gridMetricsMasonryChromeSpecs = () => {
  const g = GMMA_GEOM
  const specs = []
...
export const gridMetricsMasonryOverlay = (gx, gy, gw, gh) => {
  const g = GMMA_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToGridMetricsMasonryContent = (spec) => {
...
export const layoutGridMetricsMasonry = (elements, schema, palette = {}, canvas = {}) => {
