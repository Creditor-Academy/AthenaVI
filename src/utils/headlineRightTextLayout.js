...
export function isHeadlineRightTextLayout(layoutId, schema) {
...
export const HEADLINE_RIGHT_TEXT_DEFAULTS = {
...
export function buildHeadlineRightTextChromeSvg() {
  const {
...
export function layoutHeadlineRightText(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildHeadlineRightTextCanvasElements({ schema, options = {} } = {}) {
...
export function headlineRightTextPreviewSvg() {
  const chrome = buildHeadlineRightTextChromeSvg()
    .replace(/currentColor/g, '#6366F1')
    .replace(/^<svg[^>]*>/, '')
    .replace(/<\/svg>\s*$/, '')
  const headLines = ['Section', 'headline']
  const bodyLines = [
    'Supporting copy sits in a quiet card',
    'to the right of a bold headline panel.',
    'The layout stays airy and easy to scan.',
  ]
  const head = headLines
    .map((l, i) => `<text x="${GEOM.headX}" y="${GEOM.headY + 72 + i * 72}" fill="#0F172A" font-size="58" font-weight="800" font-family="system-ui, sans-serif">${l}</text>`)
    .join('')
  const body = bodyLines
    .map((l, i) => `<text x="${GEOM.bodyX}" y="${GEOM.bodyY + 48 + i * 52}" fill="#64748B" font-size="28" font-family="system-ui, sans-serif">${l}</text>`)
    .join('')
