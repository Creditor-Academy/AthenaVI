...
export const GMAS_GEOM = {
  ...
  bottomCenterPadding: 18,
  
  // Bottom center label text
  bottomCenterLabelY: 340,
  bottomCenterLabelH: 80,
  ...
  bottomRightTitleH: 30,
  bottomRightDescY: 315,
  bottomRightDescH: 110,
}

export const GMAS_COLORS = {
  cardBg: '#E2E8F0',
  cardBorder: '#CBD5E1',
...
export const isGridMetricsAsymmetricLayout = (layoutId) => {
  return /grid_metrics_asymmetric_v1$/i.test(String(layoutId || ''))
}

export const isGridMetricsAsymmetricSlot = (slotId) => {
...
export const gridMetricsAsymmetricChromeSpecs = () => {
  const g = GMAS_GEOM
  const specs = []
...
export const gridMetricsAsymmetricOverlay = (gx, gy, gw, gh) => {
  const g = GMAS_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToGridMetricsAsymmetricContent = (spec) => {
  if (spec.kind === 'card') {
...
export const layoutGridMetricsAsymmetric = (elements, schema, palette = {}, canvas = {}) => {
