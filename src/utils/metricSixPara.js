...
import { isMetricSixCardsLayout } from './metricSixCards.js'

export const isMetricSixParaLayout = (layoutId, schema = null) => {
...
export const isMetricSixParaTextSlot = (slotId) => {
...
const metricIconWithBgSvg = (id, color) => {
...
export const metricSixParaChromeSpecs = () => {
  const g = M6P_GEOM
  const specs = []
...
export const metricSixParaOverlay = (gx, gy, gw, gh) => {
  const g = M6P_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToMetricSixParaContent = (spec) => {
...
export const layoutMetricSixPara = (elements, schema, palette = {}, canvas = {}) => {
 ...
 */
export function metricSixParaPreviewSvg(previewHints = {}, theme = {}) {
