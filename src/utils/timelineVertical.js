...
export const TLV_PALETTE = ['#5BA84A', '#2EB5E0', '#E0A01A', '#E23B3B', '#2F6FBF']
...
export function isTimelineVerticalLayout(layoutId) {
  const id = String(layoutId || '')
  if (/cards/i.test(id)) return false
  return /timeline_vertical_v1$/i.test(id)
}

export function isTimelineVerticalTextSlot(slotId) {
...
export function timelineVerticalChromeSpecs() {
  const g = TLV_GEOM
  const specs = [{
    slotId: 'TLV_PILLAR',
    x: 0,
...
export function timelineVerticalOverlay(gx, gy, gw, gh) {
  const g = TLV_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineVerticalContent(spec) {
...
export function layoutTimelineVertical(elements, schema, palette = {}, canvas = {}) {
