...
export function chartTwoCardsChromeSpecs() {
  const g = CTC_GEOM
  const card1X = 20
...
export function chartTwoCardsOverlay(gx, gy, gw, gh) {
  const g = CTC_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToChartTwoCardsContent(spec) {
...
export function layoutChartTwoCards(elements, schema, palette = {}, canvas = {}) {
  ...
  const filtered = elements.filter((el) => !chromeRe.test(String(el.slotId || '')) && isChartTwoCardsTextSlot(el.slotId))
  ...
  const chrome = chartTwoCardsChromeSpecs().map((spec) => {
    const prev = prevBySlot.get(spec.slotId.toUpperCase())
    const graphic = specToChartTwoCardsContent(spec)
    if (!graphic) return null
    return {
      id: prev?.id || newId('shp-ctc'),
      type: 'graphic',
      layer: spec.layer || 4,
      placement: {
...
export { isChartTwoCardsLayout, isChartTwoCardsTextSlot }
