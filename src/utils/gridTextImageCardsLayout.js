/**
 * Grid Text Image Cards Layout Engine (Frontend)
 * Layout IDs:
...
export const GRID_TEXT_IMAGE_CARDS_GEOM = {
  viewW: 1000,
  viewH: 560,
...
export const GRID_TEXT_IMAGE_CARDS_THEMES = [
  {
...
export const GRID_TEXT_IMAGE_CARDS_DEFAULTS = {
...
export function isGridTextImageCardsLayout(layoutId) {
 ...
 */
export function buildGalleryCardPlaceholderSvg(cardIdx, width, height, radius = 16) {
 ...
 */
export function buildSpotlightThumbSvg(width, height) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="100%" height="100%">
...
/**
 * Main layout compiler for Grid Text Image Cards (Frontend)
 */
export function layoutGridTextImageCards(docOrElements, schema, palette = {}, canvas = {}) {
...
/**
 * Pixel-perfect standalone SVG preview matching the reference image exactly
 */
export function gridTextImageCardsPreviewSvg(options = {}) {
  const geom = GRID_TEXT_IMAGE_CARDS_GEOM
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${geom.viewW} ${geom.viewH}" width="100%" height="100%">
    ...
    <g transform="translate(${geom.pointCircle.x + geom.pointCircle.w / 2 - 16}, ${geom.pointCircle.y + geom.pointCircle.h / 2 - 14})" opacity="0.6">
      <rect x="2" y="4" width="28" height="20" rx="4" fill="none" stroke="#4F46E5" stroke-width="2" />
      <circle cx="21" cy="10" r="2.5" fill="#4F46E5" />
      <path d="M 5 21 L 13 13 L 19 19 L 23 15 L 27 21 Z" fill="none" stroke="#4F46E5" stroke-width="1.8" stroke-linejoin="round" />
    </g>
