/**
 * Process Linear Business Diagram Layout
 ...
 *  - Change the color of any individual arrow
 *  - Change the color of any individual card box border
 *  - Select, recolor, replace, or delete any icon
 *  - Edit any text directly on canvas
 */

export const PROCESS_LINEAR_BUSINESS_GEOM = {
  viewW: 1000,
  viewH: 560,
...
export const PROCESS_LINEAR_DEFAULT_COLORS = [
...
export function isProcessLinearBusinessLayout(layoutId) {
...
// Inner paths for 7 crisp vector icons (using currentColor for full recolorability)
export const PROCESS_LINEAR_ICON_PATHS = [
...
/**
 * Calculates step positions and geometry for N steps (supports 3 to 7 steps).
 */
export function calculateStepGeometries(stepCount = 7) {
  const g = PROCESS_LINEAR_BUSINESS_GEOM
  const n = Math.max(2, Math.min(7, stepCount || 7))
  const padLeft = 36
  const totalUsableW = g.viewW - padLeft * 2 // 928
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    // Alternating Card
    const isOdd = i % 2 === 0 // 0-indexed: index 0 (Step 1) is odd step -> placed ABOVE
...
/**
 * Builds standalone chevron SVG (viewBox matches chevron dimensions).
 */
export function buildSingleChevronSvg(stepW, beltH, chevronDepth) {
  const midY = beltH / 2
...
/**
 * Builds standalone card box SVG with rounded corners on alternating sides.
 */
export function buildSingleCardSvg(cardW, cardH, r, isOdd, borderWidth = 2.5) {
  const inset = borderWidth / 2
  let pathD = ''
  if (isOdd) {
    // Odd: Rounded top corners, flat bottom
    ...
    // Even: Flat top, rounded bottom corners
 ...
 */
export function buildSingleIconSvg(iconPaths) {
 ...
 */
export function processLinearBusinessPreviewSvg(previewHints = {}, theme = {}) {
  const g = PROCESS_LINEAR_BUSINESS_GEOM
  const headingText = previewHints?.slots?.HEADING?.text || 'Linear Business Process Diagram'
  ...
  const stepCount = Math.max(count, 7)
  ...
  steps.forEach((step) => {
    ...
    const midY = g.beltY + g.beltH / 2
    const chevronD = `M ${step.leftX} ${g.beltY} L ${step.leftX + g.chevronDepth} ${midY} L ${step.leftX} ${g.beltY + g.beltH} L ${step.rightX} ${g.beltY + g.beltH} L ${step.rightX + g.chevronDepth} ${midY} L ${step.rightX} ${g.beltY} Z`
    const r = g.cardRadius
    let cardD = ''
    if (step.isOdd) {
      cardD = `M ${step.cardX} ${step.cardY + r} A ${r} ${r} 0 0 1 ${step.cardX + r} ${step.cardY} L ${step.cardX + step.cardW - r} ${step.cardY} A ${r} ${r} 0 0 1 ${step.cardX + step.cardW} ${step.cardY + r} L ${step.cardX + step.cardW} ${step.cardY + step.cardH} L ${step.cardX} ${step.cardY + step.cardH} Z`
    } else {
      cardD = `M ${step.cardX} ${step.cardY} L ${step.cardX + step.cardW} ${step.cardY} L ${step.cardX + step.cardW} ${step.cardY + step.cardH - r} A ${r} ${r} 0 0 1 ${step.cardX + step.cardW - r} ${step.cardY + step.cardH} L ${step.cardX + r} ${step.cardY + step.cardH} A ${r} ${r} 0 0 1 ${step.cardX} ${step.cardY + step.cardH - r} Z`
    }
    ...
    svgs += `
      <path d="${chevronD}" fill="${color}" />
      <g transform="translate(${iconX}, ${iconY})" color="#FFFFFF">
        ${buildSingleIconSvg(step.iconPaths)}
      </g>
      <path d="${cardD}" fill="#FFFFFF" stroke="${color}" stroke-width="${g.cardBorderWidth}" stroke-linejoin="round" />
    `

    const title = previewHints?.slots?.[`STEP_${step.stepNum}_TITLE`]?.text || 'Text goes here'
    const body = previewHints?.slots?.[`STEP_${step.stepNum}_BODY`]?.text || 'Lorem Ipsum is simply dummy text of the printing and typesetting industry.'
    const textX = step.cardX + g.textPaddingX
    const textW = step.cardW - g.textPaddingX * 2
    const titleY = step.cardY + g.titleOffsetY
    const bodyY = step.cardY + g.bodyOffsetY

    textLabels += `
      <text x="${textX + textW / 2}" y="${titleY + 14}" text-anchor="middle" fill="#1E293B" font-size="12.5" font-weight="800" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif">
        ${title}
      </text>
    `
    ...
    const line4 = words.slice(12, 16).join(' ')

    textLabels += `
      <text x="${textX + textW / 2}" y="${bodyY + 12}" text-anchor="middle" fill="#64748B" font-size="9" font-weight="400" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif">
        <tspan x="${textX + textW / 2}" dy="0">${line1}</tspan>
        ${line2 ? `<tspan x="${textX + textW / 2}" dy="12">${line2}</tspan>` : ''}
        ${line3 ? `<tspan x="${textX + textW / 2}" dy="12">${line3}</tspan>` : ''}
        ${line4 ? `<tspan x="${textX + textW / 2}" dy="12">${line4}</tspan>` : ''}
      </text>
    `
  })

  return `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${g.viewW} ${g.viewH}" width="100%" height="100%">
      <rect width="100%" height="100%" fill="#FFFFFF" />
      <text x="${g.headingX}" y="${g.headingY + 24}" fill="#334155" font-size="24" font-weight="800" font-family="system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif">
        ${headingText}
      </text>
      ${svgs}
      ${textLabels}
 ...
 * Emits INDIVIDUAL elements for:
 *  - HEADING (text)
 *  - STEP_n_CHEVRON (graphic) -> individually selectable & recolorable arrow
 *  - STEP_n_CARD (graphic) -> individually selectable & recolorable box border
 *  - STEP_n_ICON (graphic) -> individually selectable, recolorable & removable icon
 *  - STEP_n_TITLE (text) -> editable title
 *  - STEP_n_BODY (text) -> editable body description
 */
export function layoutProcessLinearBusiness(elements = [], schema = {}, palette = {}, canvas = {}) {
  const g = PROCESS_LINEAR_BUSINESS_GEOM
