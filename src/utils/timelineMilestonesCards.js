...
export const TLMC_PALETTE = ['#2F6FBF', '#3D9A4A', '#D4A017', '#C75B12', '#8B1E3F']

const BODY = 'Insert your desired text here. This is a sample text.'

export const TLMC_DEFAULTS = {
...
export function isTimelineMilestonesCardsLayout(layoutId) {
  return /timeline_milestones_cards_v1$/i.test(String(layoutId || ''))
}

export function isTimelineMilestonesCardsTextSlot(slotId) {
...
export function timelineMilestonesCardsChromeSpecs() {
  const g = TLMC_GEOM
  const w = cardW()
...
export function timelineMilestonesCardsOverlay(gx, gy, gw, gh) {
  const g = TLMC_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineMilestonesCardsContent(spec) {
...
export function layoutTimelineMilestonesCards(elements, schema, palette = {}, canvas = {}) {
