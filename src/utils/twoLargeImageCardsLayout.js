...
export function isTwoLargeImageCardsLayout(layoutId, schema) {
...
export const TWO_LARGE_IMAGE_CARDS_DEFAULTS = {
...
export function buildTwoLargeImageCardsChromeSvg() {
  const {
...
export function layoutTwoLargeImageCards(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildTwoLargeImageCardsCanvasElements({ schema, options = {} } = {}) {
...
export function twoLargeImageCardsPreviewSvg() {
  const { cardXs, cardY, cardW, imgPad, imgH, imgR, textPad, titleDy, bodyDy } = GEOM
  const titles = TWO_LARGE_IMAGE_CARDS_DEFAULTS.TITLES
  const bodyLines = ['Supporting copy that explains', 'the product in a few clear', 'scannable lines.']
  const photos = cardXs
    .map((x, i) => {
      const ix = x + imgPad
      ...
      const body = bodyLines
        ...
        .join('')
      return `
    <clipPath id="tlicPrevClip${i}"><rect x="${ix}" y="${iy}" width="${iw}" height="${imgH}" rx="${imgR}" /></clipPath>
    <g clip-path="url(#tlicPrevClip${i})">
      <rect x="${ix}" y="${iy}" width="${iw}" height="${imgH}" fill="url(#tlicPrevSky)" />
      <ellipse cx="${ix + 200 + i * 90}" cy="${iy + 120}" rx="100" ry="40" fill="#FFFFFF" opacity="0.95" />
      <ellipse cx="${ix + 290 + i * 90}" cy="${iy + 120}" rx="68" ry="32" fill="#FFFFFF" opacity="0.95" />
      <path d="M${ix} ${iy + 360} C${ix + 200} ${iy + 280} ${ix + 400} ${iy + 400} ${ix + 600} ${iy + 320} C${ix + 720} ${iy + 280} ${ix + 780} ${iy + 340} ${ix + iw} ${iy + 300} L${ix + iw} ${iy + imgH} L${ix} ${iy + imgH} Z" fill="url(#tlicPrevHill)" />
    </g>
    <text x="${x + textPad}" y="${cardY + titleDy + 48}" fill="#0F172A" font-size="46" font-weight="800" font-family="system-ui, sans-serif">${titles[i]}</text>
    ${body}`
    })
    .join('')
  const chrome = buildTwoLargeImageCardsChromeSvg()
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    ...
    <defs>
      <linearGradient id="tlicPrevSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8" />
        <stop offset="100%" stop-color="#D0E3EF" />
      </linearGradient>
      <linearGradient id="tlicPrevHill" x1="0" y1="0" x2="0" y2="1">
