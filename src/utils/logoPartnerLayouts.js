/**
 * Logo Partner Layouts Engine (Frontend)
 * Layout IDs:
...
export const LOGO_PARTNER_GRID_GEOM = {
  viewW: 1000,
  viewH: 560,
...
export const LOGO_PARTNER_STRIP_GEOM = {
  viewW: 1000,
  viewH: 560,
...
export const LOGO_PARTNER_THEMES = [
...
export const LOGO_PARTNER_DEFAULTS = {
...
export function isLogoPartnerGridLayout(layoutId) {
  const id = String(layoutId || '').trim().toLowerCase()
  return id === 'logo_partner_grid_v1' || id === 'logo_partner_grid'
}

export function isLogoPartnerStripLayout(layoutId) {
  const id = String(layoutId || '').trim().toLowerCase()
 ...
 */
export function buildLogoPlaceholderSvg(cardIdx, width, height, radius = 16) {
 ...
 */
export function layoutLogoPartnerGrid(docOrElements, schema, palette = {}, canvas = {}) {
 ...
 */
export function layoutLogoPartnerStrip(docOrElements, schema, palette = {}, canvas = {}) {
  const elements = Array.isArray(docOrElements) ? docOrElements : (docOrElements?.elements || [])
  const canvasW = canvas.width || docOrElements?.canvas?.width || 1920
  const canvasH = canvas.height || docOrElements?.canvas?.height || 1080
...
export const LOGO_WALL_DEFAULTS = {
  CATEGORY: 'OUR PARTNERS',
  HEADING: 'Trusted by leading teams',
}

export function isLogoWallLayout(layoutId) {
 ...
 */
export function layoutLogoWall(docOrElements, schema, palette = {}, canvas = {}) {
  const elements = Array.isArray(docOrElements) ? docOrElements : (docOrElements?.elements || [])
  const canvasW = canvas.width || docOrElements?.canvas?.width || 1920
  const canvasH = canvas.height || docOrElements?.canvas?.height || 1080
