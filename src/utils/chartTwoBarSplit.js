...
export const CTBS_PALETTE = {
  series1: '#1E40AF',  // Dark blue
  series2: '#8B5CF6',  // Purple
}

export const CTBS_DEFAULTS = {
...
export function chartTwoBarSplitChromeSpecs(chartData = {}) {
  const g = CTBS_GEOM
  ...
  const max = Math.max(...series1Values, ...series2Values, 50000)
...
export function chartTwoBarSplitOverlay(gx, gy, gw, gh) {
  const g = CTBS_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToChartTwoBarSplitContent(spec) {
...
export function layoutChartTwoBarSplit(elements, schema, palette = {}, canvas = {}, chartData = {}) {
  ...
  const filtered = elements.filter((el) => !chromeRe.test(String(el.slotId || '')) && isChartTwoBarSplitTextSlot(el.slotId))
  ...
  const chrome = chartTwoBarSplitChromeSpecs(extractedData).map((spec) => {
    const prev = prevBySlot.get(spec.slotId.toUpperCase())
    const graphic = specToChartTwoBarSplitContent(spec)
    return {
      id: prev?.id || newId('shp-ctbs'),
      type: 'graphic',
      layer: spec.layer || 4,
      placement: {
...
export function chartTwoBarSplitPreviewSvg() {
  const specs = chartTwoBarSplitChromeSpecs()
  const g = CTBS_GEOM
  const parts = specs.map((spec) => {
    const inner = specToChartTwoBarSplitContent(spec).svg
...
export { isChartTwoBarSplitLayout, isChartTwoBarSplitTextSlot }
