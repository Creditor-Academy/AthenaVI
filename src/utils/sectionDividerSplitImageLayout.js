...
export function isSectionDividerSplitImageLayout(layoutId, schema) {
...
export const SECTION_DIVIDER_SPLIT_IMAGE_DEFAULTS = {
...
export function buildSectionDividerSplitImageChromeSvg() {
...
export function layoutSectionDividerSplitImage(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildSectionDividerSplitImageCanvasElements({ schema, options = {} } = {}) {
...
export function sectionDividerSplitImagePreviewSvg() {
  const chrome = buildSectionDividerSplitImageChromeSvg()
    .replace(/currentColor/g, '#6366F1')
    .replace(/^<svg[^>]*>/, '')
    .replace(/<\/svg>\s*$/, '')
  const { imgX, imgY, imgW, imgH, headX, headY, bodyX, bodyY } = GEOM
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    <rect width="1920" height="1080" fill="#FFFFFF" />
    ${chrome}
    <defs>
      <linearGradient id="sdsiSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8" />
        <stop offset="55%" stop-color="#B7D4E8" />
        <stop offset="100%" stop-color="#D0E3EF" />
      </linearGradient>
      <linearGradient id="sdsiHill" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#8FBF8E" />
        <stop offset="100%" stop-color="#5E945D" />
      </linearGradient>
    </defs>
    <rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" fill="url(#sdsiSky)" />
    <ellipse cx="${imgX + 180}" cy="${imgY + 220}" rx="100" ry="44" fill="#FFFFFF" opacity="0.95" />
    <ellipse cx="${imgX + 270}" cy="${imgY + 220}" rx="70" ry="34" fill="#FFFFFF" opacity="0.95" />
    <path d="M${imgX} ${imgY + 620} C${imgX + 220} ${imgY + 500} ${imgX + 420} ${imgY + 580} ${imgX + 640} ${imgY + 520} C${imgX + 780} ${imgY + 480} ${imgX + 860} ${imgY + 560} ${imgX + imgW} ${imgY + 500} L${imgX + imgW} ${imgY + imgH} L${imgX} ${imgY + imgH} Z" fill="url(#sdsiHill)" />
    <text x="${headX}" y="${headY + 72}" fill="#0F172A" font-size="56" font-weight="800" font-family="system-ui, sans-serif">Next section</text>
    <text x="${bodyX}" y="${bodyY + 40}" fill="#64748B" font-size="28" font-family="system-ui, sans-serif">What this chapter covers and why it</text>
    <text x="${bodyX}" y="${bodyY + 88}" fill="#64748B" font-size="28" font-family="system-ui, sans-serif">matters before the next decision.</text>
  </svg>`
}
