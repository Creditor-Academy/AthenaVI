...
export const TLR_PALETTE = ['#1B3A5C', '#E0B025', '#E07A1A', '#2A9B8F', '#D23A3A']
...
export function timelineRoadmapChromeSpecs() {
  const g = TLR_GEOM
  const specs = [{
    slotId: 'TLR_ROAD',
    x: 0,
...
export function timelineRoadmapOverlay(gx, gy, gw, gh) {
  const g = TLR_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineRoadmapContent(spec) {
...
export function layoutTimelineRoadmap(elements, schema, palette = {}, canvas = {}) {
