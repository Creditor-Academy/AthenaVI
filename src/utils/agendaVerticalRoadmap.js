...
export function agendaVerticalRoadmapGraphicFrame(canvasW, canvasH) {
  return {
    graphicX: 0,
    graphicY: 0,
    graphicW: canvasW,
...
export function agendaVerticalRoadmapChromeSpecs() {
  const g = AGENDA_VR_GEOM
  const specs = []
...
export function agendaVerticalRoadmapOverlayPlacements(gx, gy, gw, gh) {
  const g = AGENDA_VR_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToVerticalRoadmapContent(spec) {
  return { svg: rowInlineSvg(spec), colorMode: 'recolorable', fill: spec?.color || VR_PALETTE[0].main }
}

export function agendaVerticalRoadmapPreviewSvg() {
...
export function isAgendaVerticalRoadmapLayout(layoutId, family, variant) {
  if (family === 'timeline' && variant === 'vertical') return true
  return /agenda_vertical_roadmap_v1$/i.test(String(layoutId || ''))
}

export function isAgendaVerticalRoadmapTextSlot(slotId) {
