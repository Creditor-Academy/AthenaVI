...
export const PRICING_TH_DEFAULTS = {
...
export function isPricingThreeHighlightLayout(layoutId) {
  const id = String(layoutId || '')
  if (/split/i.test(id)) return false
  return /pricing_three_highlight_v1$/i.test(id)
}

export function isPricingThreeHighlightTextSlot(slotId) {
...
export function pricingThreeHighlightChromeSpecs(palette = {}) {
  const g = PRICING_TH_GEOM
  const w = cardW()
...
export function pricingThreeHighlightOverlay(gx, gy, gw, gh) {
  const g = PRICING_TH_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToPricingThreeHighlightContent(spec) {
  return { svg: columnChromeSvg(spec), colorMode: 'recolorable', fill: spec.color }
}

export function pricingThreeHighlightPreviewSvg() {
  const specs = pricingThreeHighlightChromeSpecs()
  const g = PRICING_TH_GEOM
  const parts = specs.map((spec) => {
    const inner = specToPricingThreeHighlightContent(spec).svg
...
export function layoutPricingThreeHighlight(elements, schema, palette = {}, canvas = {}) {
