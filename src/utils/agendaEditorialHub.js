...
export const AGENDA_EDITORIAL_GEOM = {
  viewW: 1000,
  viewH: 560,
  n: 5,
  hubCx: 198,
...
export const EDITORIAL_ROW_PALETTE = ['#2F6FED', '#E57373', '#9CCC65', '#26A69A', '#42A5F5']
...
export function agendaEditorialGraphicFrame(canvasW, canvasH) {
  return {
    graphicX: 0,
    graphicY: 0,
    graphicW: canvasW,
...
export function agendaEditorialChromeSpecs() {
  const g = AGENDA_EDITORIAL_GEOM
  const { hubCy } = rowMetrics()
  const hubS = g.hubR * 2
  const specs = [
...
export function agendaEditorialOverlayPlacements(gx, gy, gw, gh) {
  const g = AGENDA_EDITORIAL_GEOM
  const { hubCy } = rowMetrics()
  const sx = gw / g.viewW
  const sy = gh / g.viewH
...
export function specToEditorialContent(spec) {
...
export function agendaEditorialPreviewSvg() {
...
export function isAgendaEditorialLayout(layoutId, family, variant) {
  if (family === 'minimal' && variant === 'editorial') return true
  return /agenda_editorial_v1$/i.test(String(layoutId || ''))
}

export function isAgendaEditorialTextSlot(slotId) {
