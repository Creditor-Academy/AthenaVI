...
export const BULLET_LIST_SPLIT_GEOM = {
  viewW: 1000,
  viewH: 560,
  headingX: 52,
  headingY: 24,
...
export const BULLET_LIST_SPLIT_CARDS = [
  { fill: '#148A80', icon: 'list' },
  { fill: '#2F4F86', icon: 'flag' },
]

export const BULLET_LIST_SPLIT_DEFAULTS = {
...
export function isBulletListSplitLayout(layoutId) {
...
export function buildSplitCardSvg({ width, height, radius, headerH, rows, rowGap, listStart, dotX, icon, includeCopy = false, title = '', items = [] }) {
  const r = radius
  const dots = []
...
export function layoutBulletListSplit(docOrElements, schema, palette = {}, canvas = {}) {
...
export function bulletListSplitPreviewSvg() {
  const g = BULLET_LIST_SPLIT_GEOM
