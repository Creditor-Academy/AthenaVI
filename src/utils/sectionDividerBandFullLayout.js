...
export function isSectionDividerBandFullLayout(layoutId, schema) {
...
export const SECTION_DIVIDER_BAND_FULL_DEFAULTS = {
...
export function buildSectionDividerBandFullChromeSvg() {
...
export function layoutSectionDividerBandFull(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildSectionDividerBandFullCanvasElements({ schema, options = {} } = {}) {
...
export function sectionDividerBandFullPreviewSvg() {
  const chrome = buildSectionDividerBandFullChromeSvg()
    .replace(/currentColor/g, '#6366F1')
    .replace(/^<svg[^>]*>/, '')
    .replace(/<\/svg>\s*$/, '')
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    ${chrome}
    <text x="960" y="600" text-anchor="middle" fill="#FFFFFF" font-size="64" font-weight="800" font-family="system-ui, sans-serif">Section break</text>
  </svg>`
}
