...
export const isMetricThreeVerticalCardsLayout = (layoutId, schema = null) => {
...
export const isMetricThreeVerticalLayout = (layoutId, schema = null) => {
  const id = String(layoutId || schema?.layout_id || schema?.id || schema?.layoutId || '').toLowerCase()
...
export const isMetricThreeVerticalTextSlot = (slotId) => {
...
let _idCounter = 0
const newId = (prefix = 'm3v') => `${prefix}-${Date.now().toString(36)}-${(++_idCounter).toString(36)}`

const filledContent = (prev, slotId, fallbackStyle) => {
...
export const metricThreeVerticalChromeSpecs = (isCards = false) => {
  const g = M3V_GEOM
  const specs = []
...
export const metricThreeVerticalOverlay = (gx, gy, gw, gh) => {
  const g = M3V_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToMetricThreeVerticalContent = (spec) => {
...
const buildLayout = (elements, schema, palette = {}, canvas = {}, isCards = false) => {
...
export const layoutMetricThreeVertical = (elements, schema, palette = {}, canvas = {}) => {
  return buildLayout(elements, schema, palette, canvas, false)
}

export const layoutMetricThreeVerticalCards = (elements, schema, palette = {}, canvas = {}) => {
  return buildLayout(elements, schema, palette, canvas, true)
}

/**
 * Polished SVG Preview for Metric Three Vertical thumbnail in slide picker.
 * Exact 1000x560 slide canvas matching rendered slide layout.
 */
export function metricThreeVerticalPreviewSvg(previewHints = {}, theme = {}, isCards = false) {
...
export function metricThreeVerticalCardsPreviewSvg(previewHints = {}, theme = {}) {
