...
export const TLMI_PALETTE = ['#1B3A5F', '#2E6BA6', '#2F9BB0', '#5AB3D0', '#7EC6DC', '#F08A2A']
...
export function isTimelineMilestonesImageLayout(layoutId) {
  const id = String(layoutId || '')
  if (/right|top/i.test(id)) return false
  return /timeline_milestones_image_v1$/i.test(id)
}

export function isTimelineMilestonesImageTextSlot(slotId) {
...
export function timelineMilestonesImageChromeSpecs() {
  const g = TLMI_GEOM
  const w = cardW()
  const specs = []
  const framePad = 5
...
export function timelineMilestonesImageOverlay(gx, gy, gw, gh) {
  const g = TLMI_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineMilestonesImageContent(spec) {
...
export function layoutTimelineMilestonesImage(elements, schema, palette = {}, canvas = {}) {
