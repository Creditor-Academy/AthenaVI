...
export const CSB_PALETTE = ['#1E40AF', '#DC2626', '#F97316', '#7C3AED', '#0891B2', '#059669', '#84CC16']

const LOREM = 'This is a sample text. Insert your desired text here.'

export const CSB_DEFAULTS = {
...
export function chartSingleBarChromeSpecs(chartValues = []) {
  const g = CSB_GEOM
...
export function chartSingleBarOverlay(gx, gy, gw, gh) {
  const g = CSB_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToChartSingleBarContent(spec) {
...
export function layoutChartSingleBar(elements, schema, palette = {}, canvas = {}, chartData = {}) {
...
export function chartSingleBarPreviewSvg() {
  const specs = chartSingleBarChromeSpecs()
  const g = CSB_GEOM
  const parts = specs.map((spec) => {
    const inner = specToChartSingleBarContent(spec).svg
...
export { isChartSingleBarLayout, isChartSingleBarTextSlot }
