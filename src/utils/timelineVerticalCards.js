...
export const TLVC_PALETTE = ['#2F6FBF', '#E0564A', '#E0A01A', '#3D9A5F', '#6B4C9A']
...
export function isTimelineVerticalCardsLayout(layoutId) {
  return /timeline_vertical_cards_v1$/i.test(String(layoutId || ''))
}

export function isTimelineVerticalCardsTextSlot(slotId) {
...
export function timelineVerticalCardsChromeSpecs() {
  const g = TLVC_GEOM
  const specs = [{
    slotId: 'TLVC_SPINE',
    x: 0,
...
export function timelineVerticalCardsOverlay(gx, gy, gw, gh) {
  const g = TLVC_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineVerticalCardsContent(spec) {
...
export function layoutTimelineVerticalCards(elements, schema, palette = {}, canvas = {}) {
