...
export function isFullBgImageOverlaySideLayout(layoutId, schema) {
...
export const FULL_BG_IMAGE_OVERLAY_SIDE_DEFAULTS = {
...
export function buildFullBgImageOverlaySideChromeSvg() {
...
export function layoutFullBgImageOverlaySide(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildFullBgImageOverlaySideCanvasElements({ schema, options = {} } = {}) {
...
export function fullBgImageOverlaySidePreviewSvg() {
  const chrome = buildFullBgImageOverlaySideChromeSvg()
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    <defs>
      <linearGradient id="fbiosSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8" />
        <stop offset="55%" stop-color="#B7D4E8" />
        <stop offset="100%" stop-color="#D0E3EF" />
      </linearGradient>
      <linearGradient id="fbiosHill" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#8FBF8E" />
        <stop offset="100%" stop-color="#5E945D" />
      </linearGradient>
    </defs>
    <rect width="1920" height="1080" fill="url(#fbiosSky)" />
    <ellipse cx="1280" cy="200" rx="140" ry="52" fill="#FFFFFF" opacity="0.92" />
    <ellipse cx="1400" cy="200" rx="90" ry="40" fill="#FFFFFF" opacity="0.92" />
    <path d="M720 620 C1080 500 1320 640 1620 520 C1760 460 1860 560 1920 500 L1920 1080 L720 1080 Z" fill="url(#fbiosHill)" />
    ${chrome}
    <text x="${titleX}" y="${titleY + 80}" fill="#FFFFFF" font-size="52" font-weight="800" font-family="system-ui, sans-serif">Presentation title</text>
    <text x="${subX}" y="${subY + 42}" fill="rgba(255,255,255,0.88)" font-size="26" font-weight="600" font-family="system-ui, sans-serif">Supporting line or tagline</text>
    <text x="${bodyX}" y="${bodyY + 36}" fill="rgba(255,255,255,0.78)" font-size="22" font-family="system-ui, sans-serif">Copy reads from a left panel</text>
    <text x="${bodyX}" y="${bodyY + 72}" fill="rgba(255,255,255,0.78)" font-size="22" font-family="system-ui, sans-serif">while the photo stays open on the right.</text>
  </svg>`
}
