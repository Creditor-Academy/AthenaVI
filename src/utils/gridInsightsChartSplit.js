...
export const isGridInsightsChartSplitLayout = (layoutId) => {
  return /grid_insights_chart_split_v1$/i.test(String(layoutId || ''))
}

export const isGridInsightsChartSplitSlot = (slotId) => {
...
export const gridInsightsChartSplitChromeSpecs = () => {
  const g = GICS_GEOM
  const specs = []
...
export const gridInsightsChartSplitOverlay = (gx, gy, gw, gh) => {
  const g = GICS_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToGridInsightsChartSplitContent = (spec) => {
  if (spec.kind === 'leftBg') {
...
export const layoutGridInsightsChartSplit = (elements, schema, palette = {}, canvas = {}) => {
