/**
 * Para Title Right Image Boxed
 * Layout ID: para_title_right_image_boxed_v1
 *
 * Text left, square rounded photo right with soft halo.
 * Same slots as the left boxed twin.
 */

export function isParaTitleRightImageBoxedLayout(layoutId, schema) {
...
export const PARA_TITLE_RIGHT_IMAGE_BOXED_DEFAULTS = {
...
export function buildParaTitleRightImageBoxedChromeSvg() {
...
export function layoutParaTitleRightImageBoxed(docOrElements, schema = {}, palette = {}, canvas = {}) {
...
export function buildParaTitleRightImageBoxedCanvasElements({ schema, options = {} } = {}) {
  ...
  return buildElements({
    canvasW,
    canvasH,
    headingText: String(contentBySlotId.HEADING || content.title || PARA_TITLE_RIGHT_IMAGE_BOXED_DEFAULTS.HEADING).trim(),
    bodyText: String(contentBySlotId.BODY || content.body || PARA_TITLE_RIGHT_IMAGE_BOXED_DEFAULTS.BODY).trim(),
    imageUrl:
...
export function paraTitleRightImageBoxedPreviewSvg() {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 1080" width="100%" height="100%">
    ...
    <defs>
      <filter id="ptribPrevShadow" x="-12%" y="-10%" width="124%" height="128%">
        <feDropShadow dx="0" dy="18" stdDeviation="22" flood-color="#94A3B8" flood-opacity="0.22" />
      </filter>
      <clipPath id="ptribClip"><rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" rx="${radius}" /></clipPath>
      <linearGradient id="ptribSky" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#7EC8E8" />
        <stop offset="55%" stop-color="#B7D4E8" />
        <stop offset="100%" stop-color="#D0E3EF" />
      </linearGradient>
      <linearGradient id="ptribHill" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#8FBF8E" />
        <stop offset="100%" stop-color="#5E945D" />
      </linearGradient>
    </defs>
    <rect x="${frameX}" y="${frameY}" width="${frameW}" height="${frameH}" rx="${frameR}" fill="#FFFFFF" filter="url(#ptribPrevShadow)" />
    <g clip-path="url(#ptribClip)">
      <rect x="${imgX}" y="${imgY}" width="${imgW}" height="${imgH}" fill="url(#ptribSky)" />
      <ellipse cx="${imgX + 180}" cy="${imgY + 150}" rx="80" ry="38" fill="#FFFFFF" opacity="0.95" />
      <ellipse cx="${imgX + 240}" cy="${imgY + 150}" rx="58" ry="30" fill="#FFFFFF" opacity="0.95" />
      <ellipse cx="${imgX + 460}" cy="${imgY + 120}" rx="96" ry="42" fill="#FFFFFF" opacity="0.95" />
      <path d="M${imgX} ${imgY + 390} C${imgX + 160} ${imgY + 300} ${imgX + 300} ${imgY + 360} ${imgX + 440} ${imgY + 340} C${imgX + 560} ${imgY + 324} ${imgX + 640} ${imgY + 380} ${imgX + imgW} ${imgY + 330} L${imgX + imgW} ${imgY + imgH} L${imgX} ${imgY + imgH} Z" fill="url(#ptribHill)" />
    </g>
    ...
    <text x="${textX}" y="540" fill="#64748B" font-size="20" font-family="system-ui, sans-serif">We help teams turn complex ideas into</text>
    <text x="${textX}" y="572" fill="#64748B" font-size="20" font-family="system-ui, sans-serif">clear narratives that drive decisions and</text>
    <text x="${textX}" y="604" fill="#64748B" font-size="20" font-family="system-ui, sans-serif">build momentum across the</text>
    <text x="${textX}" y="636" fill="#64748B" font-size="20" font-family="system-ui, sans-serif">organization.</text>
  </svg>`
}
