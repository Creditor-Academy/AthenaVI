...
export function chartTwoBarChromeSpecs(chartData = {}) {
  const g = CTB_GEOM
...
export function chartTwoBarOverlay(gx, gy, gw, gh) {
  const g = CTB_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToChartTwoBarContent(spec) {
...
export function layoutChartTwoBar(elements, schema, palette = {}, canvas = {}, chartData = {}) {
  ...
  const filtered = elements.filter((el) => !chromeRe.test(String(el.slotId || '')) && isChartTwoBarTextSlot(el.slotId))
  ...
  const chrome = chartTwoBarChromeSpecs(extractedData).map((spec) => {
    const prev = prevBySlot.get(spec.slotId.toUpperCase())
    const graphic = specToChartTwoBarContent(spec)
    return {
      id: prev?.id || newId('shp-ctb'),
      type: 'graphic',
      layer: spec.layer || 4,
      placement: {
...
export function chartTwoBarPreviewSvg() {
  const specs = chartTwoBarChromeSpecs()
  const g = CTB_GEOM
  const parts = specs.map((spec) => {
    const inner = specToChartTwoBarContent(spec).svg
...
export { isChartTwoBarLayout, isChartTwoBarTextSlot }
