/**
 * Table with Description Side Layout
...
export const TABLE_SIDE_PALETTE = {
  colHeaders: [
...
export const TABLE_SIDE_DEFAULTS = {
  HEADING: 'Table Slide',
...
export function isTableWithDescriptionSideLayout(layoutId) {
  const s = String(layoutId || '').toLowerCase()
  return s === 'table_with_description_side_v1' || s === 'table_side_desc'
}

/**
 * 4 Outline vector icons for column tops
 */
const COL_ICON_SVGS = [
...
/**
 * Builds the vector SVG chrome (column icons, headers, card cells without dots)
 */
export function buildTableSideSvgChrome(palette = TABLE_SIDE_PALETTE) {
  ...
  for (let c = 0; c < g.cols; c += 1) {
    ...
    // Outline Icon centered above column header
    svgParts.push(`
      <g transform="translate(${colX + (g.colW - 28) / 2}, ${g.iconY})">
        ${iconSvg}
      </g>
      <!-- Column ${c + 1} Header Card -->
...
export function tableSideChromeSpecs() {
  const g = TABLE_SIDE_GEOM
  return [
    {
...
export function tableSideOverlay(canvasW, canvasH) {
  const g = TABLE_SIDE_GEOM
...
export function specToTableSideContent(spec, palette) {
  return {
...
export function tableSidePreviewSvg() {
  return buildTableSideSvgChrome(TABLE_SIDE_PALETTE)
}

/**
 * Main Layout Compiler for Table with Description Side (Table on LEFT, Desc on RIGHT)
 */
export function layoutTableWithDescriptionSide(elements, schema, palette = {}, canvas = {}) {
