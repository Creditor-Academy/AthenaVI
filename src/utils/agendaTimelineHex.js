...
export const AGENDA_TL_HEX_GEOM = {
  viewW: 1000,
  viewH: 560,
  n: 6,
  padX: 22,
...
export function agendaTimelineHexGraphicFrame(canvasW, canvasH) {
  return {
    graphicX: 0,
    graphicY: 0,
    graphicW: canvasW,
...
export function agendaTimelineHexChromeSpecs() {
  const g = AGENDA_TL_HEX_GEOM
...
export function agendaTimelineHexOverlayPlacements(gx, gy, gw, gh) {
  const g = AGENDA_TL_HEX_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineHexContent(spec) {
...
export function agendaTimelineHexPreviewSvg() {
...
export function isAgendaTimelineHexLayout(layoutId, family, variant) {
...
export function isAgendaTimelineHexTextSlot(slotId) {
