...
export const isGridThreeImagesTextLayout = (layoutId) => {
  return /grid_three_images_text_v1$/i.test(String(layoutId || ''))
}

export const isGridThreeImagesTextSlot = (slotId) => {
...
export const gridThreeImagesTextChromeSpecs = () => {
  const g = GTIT_GEOM
  const specs = []
...
export const gridThreeImagesTextOverlay = (gx, gy, gw, gh) => {
  const g = GTIT_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToGridThreeImagesTextContent = (spec) => {
...
export const layoutGridThreeImagesText = (elements, schema, palette = {}, canvas = {}) => {
