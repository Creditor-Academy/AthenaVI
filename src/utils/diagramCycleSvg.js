...
export const CYCLE_SEGMENT_COLORS = ['#3B82F6', '#22C55E', '#14B8A6', '#6366F1']

export const CYCLE_GEOM = {
  view: 1000,
  rOut: 390,
  rIn: 228,
  hubR: 196,
...
export function cycleSegmentMidAngle(index) {
...
/** Canvas placements for editable numbers + hub label, matching the ring SVG. */
export function cycleOverlayPlacements(cycleX, cycleY, cycleSize) {
...
export function cycleSegmentPath(index) {
  const i = Number(index) || 0
...
/** Cropped viewBoxes so each pit is its own clickable element. */
export const CYCLE_SEG_VIEWS = [
...
export function cycleSegmentPlacement(cycleX, cycleY, cycleSize, index) {
...
export function cycleSegmentInlineSvg(index) {
  const i = Number(index) || 0
...
export function buildCycleDiagramSvg(colors = CYCLE_SEGMENT_COLORS, { labels = true } = {}) {
  const cx = 500
  const cy = 500
...
export function cycleNodePalette(index = 0) {
...
export function cycleNodeTopArcSvg() {
...
export function cycleNodeBotArcSvg() {
...
export function cycleNodeIconSvg(index = 0) {
  const i = ((Number(index) % 5) + 5) % 5
  if (i === 0) {
...
export const CYCLE_HBAR_COLORS = CYCLE_NODE_ACCENTS

/** Full-slide loop: top arc + left-pointing head, plus a small entry arrow on the left. */
export function cycleLoopInlineSvg() {
...
export function cycleBarInlineSvg(kind = 'mid') {
  if (kind === 'first') {
...
export function cycleDropInlineSvg() {
...
export function cycleChevronInlineSvg() {
...
export function cycleHbarIconSvg(index = 0) {
  const i = ((Number(index) % 5) + 5) % 5
  if (i === 0) {
    return hbarIconWrap(
...
export function cycleDiagramInlineSvg(colors, options = { labels: false }) {
