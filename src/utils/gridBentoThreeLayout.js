 ...
 */
export function layoutGridBentoThree(docOrElements = [], schema = {}, palette = {}, canvas = {}) {
...
export function gridBentoThreePreviewSvg(options = {}) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${geom.viewW} ${geom.viewH}" width="100%" height="100%">
    ...
    <!-- Card 1: Top Hero Bento (Full Image) -->
    <g>
      ...
      <g transform="translate(${geom.card1.imgX}, ${geom.card1.imgY})">
        <rect width="${geom.card1.imgW}" height="${geom.card1.imgH}" rx="${geom.card1.imgRadius}" fill="#E6FFFA" stroke="#CCFBF1" stroke-width="1.2" />
        <line x1="${geom.card1.imgW * 0.33}" y1="8" x2="${geom.card1.imgW * 0.33}" y2="${geom.card1.imgH - 8}" stroke="#0D9488" stroke-width="1" stroke-dasharray="3,3" opacity="0.25" />
        <line x1="${geom.card1.imgW * 0.66}" y1="8" x2="${geom.card1.imgW * 0.66}" y2="${geom.card1.imgH - 8}" stroke="#0D9488" stroke-width="1" stroke-dasharray="3,3" opacity="0.25" />
        <g transform="translate(${geom.card1.imgW / 2 - 16}, ${geom.card1.imgH / 2 - 20})" opacity="0.5">
          <rect x="2" y="5" width="28" height="20" rx="4" fill="none" stroke="#0D9488" stroke-width="2" />
          <circle cx="16" cy="15" r="5.5" fill="none" stroke="#0D9488" stroke-width="2" />
          <path d="M 10 5 L 12 2 L 20 2 L 22 5 Z" fill="none" stroke="#0D9488" stroke-width="2" />
        </g>
        <rect x="${geom.card1.imgW / 2 - 45}" y="${geom.card1.imgH - 30}" width="90" height="20" rx="10" fill="#FFFFFF" opacity="0.9" />
        <text x="${geom.card1.imgW / 2}" y="${geom.card1.imgH - 16}" text-anchor="middle" fill="#0D9488" font-size="9.5" font-weight="800" font-family="Inter, system-ui, sans-serif" letter-spacing="0.08em">VISUAL SLOT</text>
      </g>
    </g>

    <!-- Card 2: Bottom Left Bento (Full Image) -->
    <g>
      ...
      <g transform="translate(${geom.card2.imgX}, ${geom.card2.imgY})">
        <rect width="${geom.card2.imgW}" height="${geom.card2.imgH}" rx="${geom.card2.imgRadius}" fill="#F0F9FF" stroke="#BAE6FD" stroke-width="1.2" />
        <line x1="${geom.card2.imgW * 0.33}" y1="8" x2="${geom.card2.imgW * 0.33}" y2="${geom.card2.imgH - 8}" stroke="#0284C7" stroke-width="1" stroke-dasharray="3,3" opacity="0.25" />
        <line x1="${geom.card2.imgW * 0.66}" y1="8" x2="${geom.card2.imgW * 0.66}" y2="${geom.card2.imgH - 8}" stroke="#0284C7" stroke-width="1" stroke-dasharray="3,3" opacity="0.25" />
        <g transform="translate(${geom.card2.imgW / 2 - 16}, ${geom.card2.imgH / 2 - 20})" opacity="0.5">
          <rect x="2" y="5" width="28" height="20" rx="4" fill="none" stroke="#0284C7" stroke-width="2" />
          <circle cx="16" cy="15" r="5.5" fill="none" stroke="#0284C7" stroke-width="2" />
          <path d="M 10 5 L 12 2 L 20 2 L 22 5 Z" fill="none" stroke="#0284C7" stroke-width="2" />
        </g>
        <rect x="${geom.card2.imgW / 2 - 45}" y="${geom.card2.imgH - 30}" width="90" height="20" rx="10" fill="#FFFFFF" opacity="0.9" />
        <text x="${geom.card2.imgW / 2}" y="${geom.card2.imgH - 16}" text-anchor="middle" fill="#0284C7" font-size="9.5" font-weight="800" font-family="Inter, system-ui, sans-serif" letter-spacing="0.08em">VISUAL SLOT</text>
      </g>
    </g>

    <!-- Card 3: Bottom Right Bento (Full Image) -->
    <g>
      ...
      <g transform="translate(${geom.card3.imgX}, ${geom.card3.imgY})">
        <rect width="${geom.card3.imgW}" height="${geom.card3.imgH}" rx="${geom.card3.imgRadius}" fill="#FFF1F2" stroke="#FECDD3" stroke-width="1.2" />
        <line x1="${geom.card3.imgW * 0.33}" y1="8" x2="${geom.card3.imgW * 0.33}" y2="${geom.card3.imgH - 8}" stroke="#E11D48" stroke-width="1" stroke-dasharray="3,3" opacity="0.25" />
        <line x1="${geom.card3.imgW * 0.66}" y1="8" x2="${geom.card3.imgW * 0.66}" y2="${geom.card3.imgH - 8}" stroke="#E11D48" stroke-width="1" stroke-dasharray="3,3" opacity="0.25" />
        <g transform="translate(${geom.card3.imgW / 2 - 16}, ${geom.card3.imgH / 2 - 20})" opacity="0.5">
          <rect x="2" y="5" width="28" height="20" rx="4" fill="none" stroke="#E11D48" stroke-width="2" />
          <circle cx="16" cy="15" r="5.5" fill="none" stroke="#E11D48" stroke-width="2" />
          <path d="M 10 5 L 12 2 L 20 2 L 22 5 Z" fill="none" stroke="#E11D48" stroke-width="2" />
        </g>
        <rect x="${geom.card3.imgW / 2 - 45}" y="${geom.card3.imgH - 30}" width="90" height="20" rx="10" fill="#FFFFFF" opacity="0.9" />
        <text x="${geom.card3.imgW / 2}" y="${geom.card3.imgH - 16}" text-anchor="middle" fill="#E11D48" font-size="9.5" font-weight="800" font-family="Inter, system-ui, sans-serif" letter-spacing="0.08em">VISUAL SLOT</text>
      </g>
    </g>
  </svg>`
}
