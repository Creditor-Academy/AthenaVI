...
export const AGENDA_TWO_COL_RIBBON_GEOM = {
  viewW: 1000,
  viewH: 560,
  headingX: 40,
  headingY: 12,
...
export const TWO_COL_RIBBON_PALETTE = ['#1AA6A6', '#7CB342']
...
export function agendaTwoColumnRibbonGraphicFrame(canvasW, canvasH) {
  return {
    graphicX: 0,
    graphicY: 0,
    graphicW: canvasW,
...
export function agendaTwoColumnRibbonChromeSpecs() {
  const g = AGENDA_TWO_COL_RIBBON_GEOM
  return [0, 1].map((i) => ({
...
export function agendaTwoColumnRibbonOverlayPlacements(gx, gy, gw, gh) {
  const g = AGENDA_TWO_COL_RIBBON_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTwoColumnRibbonContent(spec) {
  return { svg: columnInlineSvg(spec?.n || 0), colorMode: 'recolorable', fill: spec.color }
}

export function agendaTwoColumnRibbonPreviewSvg() {
...
export function isAgendaTwoColumnRibbonLayout(layoutId, family, variant) {
...
export function isAgendaTwoColumnRibbonTextSlot(slotId) {
