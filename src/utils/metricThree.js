...
export const isMetricThreeLayout = (layoutId) => {
  return /metric_three_v1$/i.test(String(layoutId || ''))
}

export const isMetricThreeTextSlot = (slotId) => {
...
export const metricThreeChromeSpecs = () => {
  const g = MTH_GEOM
  const specs = []
...
export const metricThreeOverlay = (gx, gy, gw, gh) => {
  const g = MTH_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToMetricThreeContent = (spec) => {
...
export const layoutMetricThree = (elements, schema, palette = {}, canvas = {}) => {
