...
export const PRICING_FF_PALETTE = [
...
export const PRICING_FF_DEFAULTS = {
...
export function isPricingFourPlansFeaturedLayout(layoutId) {
  return /pricing_four_plans_featured_v1$/i.test(String(layoutId || ''))
}

export function isPricingFourPlansFeaturedTextSlot(slotId) {
...
export function pricingFourPlansFeaturedChromeSpecs() {
  const g = PRICING_FF_GEOM
  const w = cardW()
  const specs = []
...
export function pricingFourPlansFeaturedOverlay(gx, gy, gw, gh) {
  const g = PRICING_FF_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToPricingFourPlansFeaturedContent(spec) {
  return { svg: cardChromeSvg(spec), colorMode: 'recolorable', fill: spec.color }
}

export function pricingFourPlansFeaturedPreviewSvg() {
  const specs = pricingFourPlansFeaturedChromeSpecs()
  const g = PRICING_FF_GEOM
  const parts = specs.map((spec) => {
    const inner = specToPricingFourPlansFeaturedContent(spec).svg
...
export function layoutPricingFourPlansFeatured(elements, schema, palette = {}, canvas = {}) {
