...
export const TLM_PALETTE = ['#1A6B6B', '#2A9B8F', '#7A8F3A', '#E08A2A', '#D4453A', '#8B2E2E']
...
export function isTimelineMilestonesLayout(layoutId) {
  const id = String(layoutId || '')
  if (/cards|image/i.test(id)) return false
  return /timeline_milestones_v1$/i.test(id)
}

export function isTimelineMilestonesTextSlot(slotId) {
...
export function timelineMilestonesChromeSpecs() {
  const g = TLM_GEOM
  const w = cardW()
  const specs = [{
    slotId: 'TLM_SPINE',
...
export function timelineMilestonesOverlay(gx, gy, gw, gh) {
  const g = TLM_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineMilestonesContent(spec) {
...
export function layoutTimelineMilestones(elements, schema, palette = {}, canvas = {}) {
