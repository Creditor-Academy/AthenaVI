...
export const PRICING_FP_PALETTE = ['#4EC4F5', '#F39C12', '#4EC4F5', '#4EC4F5']

export const PRICING_FP_DEFAULTS = {
...
export function isPricingFourPlansLayout(layoutId) {
  const id = String(layoutId || '')
  if (/featured/i.test(id)) return false
  return /pricing_four_plans_v1$/i.test(id)
}

export function isPricingFourPlansTextSlot(slotId) {
...
export function pricingFourPlansChromeSpecs() {
  const g = PRICING_FP_GEOM
  const w = cardW()
  const specs = []
...
export function pricingFourPlansOverlay(gx, gy, gw, gh) {
  const g = PRICING_FP_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToPricingFourPlansContent(spec) {
...
export function pricingFourPlansPreviewSvg() {
  const specs = pricingFourPlansChromeSpecs()
  const g = PRICING_FP_GEOM
  const parts = specs.map((spec) => {
    const inner = specToPricingFourPlansContent(spec).svg
...
export function layoutPricingFourPlans(elements, schema, palette = {}, canvas = {}) {
