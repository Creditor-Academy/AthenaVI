...
export function isPricingThreeHighlightSplitLayout(layoutId) {
  return /pricing_three_highlight_split_v1$/i.test(String(layoutId || ''))
}

export function isPricingThreeHighlightSplitTextSlot(slotId) {
...
export function pricingThreeHighlightSplitChromeSpecs() {
  const g = PRICING_HS_GEOM
  const w = planW()
  const specs = [{
...
export function pricingThreeHighlightSplitOverlay(gx, gy, gw, gh) {
  const g = PRICING_HS_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToPricingThreeHighlightSplitContent(spec) {
...
export function pricingThreeHighlightSplitPreviewSvg() {
  const specs = pricingThreeHighlightSplitChromeSpecs()
  const g = PRICING_HS_GEOM
  const parts = specs.map((spec) => {
    const inner = specToPricingThreeHighlightSplitContent(spec).svg
...
export function layoutPricingThreeHighlightSplit(elements, schema, palette = {}, canvas = {}) {
