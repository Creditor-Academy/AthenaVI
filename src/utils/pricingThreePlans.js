...
export const PRICING_TP_DEFAULTS = {
...
export function pricingThreePlansChromeSpecs() {
  const g = PRICING_TP_GEOM
  const w = cardW()
  const specs = [
    {
...
export function pricingThreePlansOverlay(gx, gy, gw, gh) {
  const g = PRICING_TP_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
...
export function specToPricingThreePlansContent(spec) {
...
export function pricingThreePlansPreviewSvg() {
...
export function layoutPricingThreePlans(elements, schema, palette = {}, canvas = {}) {
