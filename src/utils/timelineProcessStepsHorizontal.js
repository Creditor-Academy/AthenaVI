...
export const TLPS_PALETTE = ['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#6B7280']

const LOREM = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.'

export const TLPS_DEFAULTS = {
...
export function timelineProcessStepsHorizontalChromeSpecs() {
  const g = TLPS_GEOM
  const specs = [{
    slotId: 'TLPS_LINE',
    x: 0,
...
export function timelineProcessStepsHorizontalOverlay(gx, gy, gw, gh) {
  const g = TLPS_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export function specToTimelineProcessStepsHorizontalContent(spec) {
...
export function layoutTimelineProcessStepsHorizontal(elements, schema, palette = {}, canvas = {}) {
...
export function timelineProcessStepsHorizontalPreviewSvg() {
  const specs = timelineProcessStepsHorizontalChromeSpecs()
  const g = TLPS_GEOM
  const parts = specs.map((spec) => {
    const inner = specToTimelineProcessStepsHorizontalContent(spec).svg
...
export { isTimelineProcessStepsHorizontalLayout, isTimelineProcessStepsHorizontalTextSlot }
