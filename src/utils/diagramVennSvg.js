export const VENN_N = 3
export const VENN_COLORS = ['#4a6fa5', '#e08a2a', '#2a9d8f']
...
export function vennRingColor(base, ring) {
...
export function vennCoreInlineSvg() {
...
export function vennSetGeom(i, frame) {
  const { outer, mid, core, gap, cy, startX, colW, colPad, insetX, titleY, titleH, bodyY, bodyH } =
    frame
...
export function vennPreviewSvg(colors = VENN_COLORS) {
  const w = 900
  const h = 320
  const outer = 118
  const mid = 78
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    parts.push(`<circle cx="${cx}" cy="${cy}" r="${outer}" fill="${vennRingColor(colors[i], 'outer')}" opacity="0.85"/>`)
    parts.push(`<circle cx="${cx}" cy="${cy}" r="${mid}" fill="${vennRingColor(colors[i], 'mid')}" opacity="0.92"/>`)
    parts.push(`<circle cx="${cx}" cy="${cy}" r="${core}" fill="${vennRingColor(colors[i], 'core')}"/>`)
  }
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">${parts.join('')}</svg>`
}

export function vennModeFromSchema(schema) {
...
export function vennThreeCirclePreviewSvg(colors = VENN_COLORS) {
  const r = 70
  const d = 78
  ...
  const cy0 = 86
  const parts = [
    `<circle cx="${cx0}" cy="${cy0}" r="${r}" fill="${colors[0]}" opacity="0.62"/>`,
    `<circle cx="${cx0 + d}" cy="${cy0}" r="${r}" fill="${colors[1]}" opacity="0.62"/>`,
    `<circle cx="${cx0 + d / 2}" cy="${cy0 + triH}" r="${r}" fill="${colors[2]}" opacity="0.62"/>`,
  ]
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 260" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">${parts.join('')}</svg>`
}

export function vennStackedPreviewSvg(colors = VENN_COLORS) {
  const r = 48
  const overlap = 36
  const cx = 70
  const y0 = 54
  const parts = [0, 1, 2].map((i) => `<circle cx="${cx}" cy="${y0 + i * overlap}" r="${r}" fill="${colors[i]}" opacity="0.62"/>`)
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 140 220" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">${parts.join('')}</svg>`
}

export function vennThreeCircleFrame(canvasW, canvasH) {
  const headingY = 52
  const headingH = 80
  ...
  const labels = [
    ...
    { title: { x: Math.round(rightX), y: Math.round(c[1].cy - 48), width: rightW, height: 48 }, body: { x: Math.round(rightX), y: Math.round(c[1].cy + 4), width: rightW, height: 88 } },
...
export function vennStackedFrame(canvasW, canvasH) {
  const headingY = 56
  const headingH = 88
