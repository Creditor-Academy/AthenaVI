/**
 * Chart three context — ONE chart with 3 metrics on left + context panel on right.
 * Layout id: chart_three_context_v1.
 */

export const CTXC_GEOM = {
  viewW: 1000,
  viewH: 560,
...
// Total: 9 bars (3 quarters × 3 bars each)
// Height = value * 2.6 (scale: 0-100 → 0-260px)

export const CTXC_COLORS = {
  metricA: '#3B82F6',  // Blue
  metricB: '#8B5CF6',  // Purple  
  metricC: '#10B981',  // Teal/Green
}

export const CTXC_DEFAULTS = {
...
export const isChartThreeContextLayout = (layoutId) => {
  return /chart_three_context_v1$/i.test(String(layoutId || ''))
}

export const isChartThreeContextTextSlot = (slotId) => {
...
const barSvg = (width, height, color) => {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="100%" height="100%" preserveAspectRatio="none">
    <defs>
      <linearGradient id="barGrad_${color.replace('#','')}" x1="0%" y1="0%" x2="0%" y2="100%">
        <stop offset="0%" style="stop-color:${color};stop-opacity:1" />
        <stop offset="100%" style="stop-color:${color};stop-opacity:0.85" />
      </linearGradient>
    </defs>
    <rect x="0" y="0" width="${width}" height="${height}" fill="url(#barGrad_${color.replace('#','')})" rx="6"/>
  </svg>`
}

const legendDotSvg = (color) => {
...
export const chartThreeContextChromeSpecs = () => {
  const g = CTXC_GEOM
  const specs = []
  ...
  // CUSTOM SVG BARS (9 bars total - 3 per quarter) with value labels
  CTXC_BAR_DATA.forEach((bar, i) => {
...
export const chartThreeContextOverlay = (gx, gy, gw, gh) => {
  const g = CTXC_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToChartThreeContextContent = (spec) => {
...
export const layoutChartThreeContext = (elements, schema, palette = {}, canvas = {}) => {
  ...
  const filtered = elements.filter((el) => !chromeRe.test(String(el.slotId || '')) && isChartThreeContextTextSlot(el.slotId))
  ...
  const chrome = chartThreeContextChromeSpecs().map((spec) => {
    const prev = prevBySlot.get(spec.slotId.toUpperCase())
    const graphic = specToChartThreeContextContent(spec)
    if (!graphic) return null
    return {
      id: prev?.id || newId('shp-ctxc'),
      type: 'graphic',
      layer: spec.layer || 4,
      placement: {
