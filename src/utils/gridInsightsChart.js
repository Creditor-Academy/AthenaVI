...
export const isGridInsightsChartLayout = (layoutId) => {
  return /grid_insights_chart_v1$/i.test(String(layoutId || ''))
}

export const isGridInsightsChartSlot = (slotId) => {
...
export const gridInsightsChartChromeSpecs = () => {
  const g = GIC_GEOM
  const specs = []
...
export const gridInsightsChartOverlay = (gx, gy, gw, gh) => {
  const g = GIC_GEOM
  const sx = gw / g.viewW
  const sy = gh / g.viewH
  const box = (x, y, w, h) => ({
...
export const specToGridInsightsChartContent = (spec) => {
...
export const layoutGridInsightsChart = (elements, schema, palette = {}, canvas = {}) => {
