...
export const AGENDA_NUMBERED_TL_GEOM = {
  viewW: 1000,
  viewH: 560,
  headingY: 16,
  headingH: 42,
...
export const NUMBERED_TL_PALETTE = [
...
export function agendaNumberedTimelineGraphicFrame(canvasW, canvasH) {
  return {
    graphicX: 0,
    graphicY: 0,
    graphicW: canvasW,
...
export function agendaNumberedTimelineNodeInlineSvg(spec) {
  const w = spec.w
  const h = spec.h
...
export function agendaNumberedTimelineInlineSvg() {
...
export function agendaNumberedTimelineChromeSpecs() {
...
export function agendaNumberedTimelineOverlayPlacements(gx, gy, gw, gh) {
...
export function agendaNumberedTimelinePreviewSvg() {
...
export function specToNumberedTimelineContent(spec) {
  if (spec?.axis) {
...
export function isAgendaNumberedTimelineLayout(layoutId, family, variant) {
  if (family === 'numbered' && (variant === 'path' || variant === 'timeline')) return true
  return /agenda_numbered_timeline/i.test(String(layoutId || ''))
}

export function isAgendaNumberedTimelineTextSlot(slotId) {
