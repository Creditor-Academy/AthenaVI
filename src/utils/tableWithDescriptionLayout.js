 ...
 *  - Theme responsive: Uses professional corporate Navy/Slate default, dynamically recolorable via theme palette
 */

export const TABLE_WITH_DESC_GEOM = {
  viewW: 1000,
  viewH: 560,
...
export const TABLE_WITH_DESC_PALETTE = {
...
export const TABLE_WITH_DESC_DEFAULTS = {
...
export function isTableWithDescriptionLayout(layoutId) {
...
/**
 * Builds the complete SVG vector chrome for Table with Description
 */
export function buildTableWithDescriptionSvgChrome(palette = TABLE_WITH_DESC_PALETTE) {
  ...
  // 2. Left Row Column Background Plate (gives layered 3D depth)
...
export function tableWithDescriptionChromeSpecs() {
  const g = TABLE_WITH_DESC_GEOM
  return [
    {
...
export function tableWithDescriptionOverlay(canvasW, canvasH) {
  const g = TABLE_WITH_DESC_GEOM
  ...
  // 6 Row Labels (inside the pill to the right of icon)
  const rowLabels = []
  for (let r = 0; r < g.rows; r += 1) {
    const rowY = g.tableY + 52 + r * (g.rowH + g.rowGap)
    rowLabels.push({
...
export function specToTableWithDescriptionContent(spec, palette) {
  return {
...
export function tableWithDescriptionPreviewSvg() {
  return buildTableWithDescriptionSvgChrome(TABLE_WITH_DESC_PALETTE)
}

/**
 * Main Layout Compiler for Table with Description
 */
export function layoutTableWithDescription(elements, schema, palette = {}, canvas = {}) {
  ...
  // Index existing incoming elements by uppercase slotId
  ...
  // 2. Description (reads from BODY or DESCRIPTION, fits in 2-3 lines without spilling)
  ...
  // 3. 4 Column Headers (single line centered)
  ...
  // 4. 6 Row Labels (single line centered in pill)
  ...
  // 5. 24 Data Cells (6 rows x 4 cols, compact 2-line centered)
  for (let r = 0; r < TABLE_WITH_DESC_GEOM.rows; r += 1) {
    const rowNum = r + 1
    for (let c = 0; c < TABLE_WITH_DESC_GEOM.cols; c += 1) {
      const colNum = c + 1
