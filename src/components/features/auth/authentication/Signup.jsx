...
function Signup({
  ...
  const handleEmailExists = (precheckOrResult) => {
    if (inviteMode && onSwitchToLogin) {
      onSwitchToLogin()
      return true
    }
    setError(formatAuthErrorMessage(precheckOrResult, 'Email already registered'))
    return true
  }

  const handleInvalidInviteError = (message) => {
    const lower = String(message || '').toLowerCase()
    if (
      inviteMode &&
      onInvalidInvite &&
      (lower.includes('invitation') || lower.includes('expired') || lower.includes('invalid'))
    ) {
      onInvalidInvite(message)
      return true
    }
    return false
  }
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      if (!precheck.success) {
        if (
          inviteMode &&
          (precheck.status === 409 || precheck.emailExists) &&
          onSwitchToLogin
        ) {
          onSwitchToLogin()
          return
        }
        if (handleInvalidInviteError(precheck.error)) return
      ...
      if (result.success) {
        ...
        if (
          inviteMode &&
          (result.status === 409 || result.emailExists) &&
          onSwitchToLogin
        ) {
          onSwitchToLogin()
          return
      ...
      if (err.emailExists && inviteMode && onSwitchToLogin) {
        onSwitchToLogin()
        return
      }
      if (handleInvalidInviteError(err.message)) return
  ...
  const handleOTPSubmit = async (event) => {
    ...
    try {
      ...
      if (result.success) {
        if (inviteMode && onInviteComplete) {
          onInviteComplete(result.workspace || null)
          return
        }
      ...
      if (result.status === 409 || isEmailAlreadyRegisteredMessage(message)) {
        if (handleEmailExists(result)) return
      }
      if (result.status === 400 && handleInvalidInviteError(message)) return
