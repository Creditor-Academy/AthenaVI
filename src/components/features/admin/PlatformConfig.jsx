...
const PlatformConfig = () => {
  ...
  return (
    <section className="admin-card-section" style={{ marginTop: '24px' }}>
      ...
      {/* ── Section 3: Payment Status Testing ──────────────────── */}
      <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '32px' }}>
        <div style={{ marginBottom: '20px' }}>
          <h2 style={{ margin: '0 0 4px' }}>Payment Status Simulator</h2>
          <p className="admin-placeholder-text">Test how the platform handles different Stripe payment outcomes before going live.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', marginBottom: '28px' }}>
          {PAYMENT_STATUSES.map(s => {
            const Icon = s.icon;
            const isActive = activePayStatus === s.id;
            return (
              <button key={s.id} onClick={() => setActivePayStatus(s.id)}
                style={{ padding: '14px 16px', borderRadius: '14px', border: `2px solid ${isActive ? s.color : 'var(--border-color)'}`, background: isActive ? s.bg : 'transparent', cursor: 'pointer', textAlign: 'left', transition: 'all 0.2s', boxShadow: isActive ? `0 0 0 3px ${s.color}22` : 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                  <Icon size={18} style={{ color: s.color }} />
                  <span style={{ fontSize: '0.85rem', fontWeight: 700, color: isActive ? s.color : 'var(--text-main)' }}>{s.label}</span>
                </div>
                <p style={{ margin: 0, fontSize: '0.75rem', color: 'var(--text-muted)', lineHeight: 1.4 }}>{s.desc}</p>
              </button>
            );
          })}
        </div>
        ...
        {/* Test Controls */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '14px', alignItems: 'flex-end', marginBottom: '20px', flexWrap: 'wrap' }}>
          <div>
            <label style={{ fontSize: '0.78rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: '6px' }}>Test User</label>
            <select value={payTestUser} onChange={e => setPayTestUser(e.target.value)}
              style={{ width: '100%', padding: '10px 14px', borderRadius: '10px', border: '1px solid var(--border-color)', background: 'var(--bg-surface)', color: 'var(--text-main)', fontSize: '0.9rem', outline: 'none' }}>
              {subscribers.map(s => <option key={s.id} value={s.id}>{s.name} ({s.currentPlan})</option>)}
            </select>
          </div>
          <div>
            <label style={{ fontSize: '0.78rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: '6px' }}>Amount ($)</label>
            <input type="number" value={payTestAmount} onChange={e => setPayTestAmount(e.target.value)}
              style={{ width: '100%', padding: '10px 14px', borderRadius: '10px', border: '1px solid var(--border-color)', background: 'var(--bg-surface)', color: 'var(--text-main)', fontSize: '0.9rem', outline: 'none', boxSizing: 'border-box' }} />
          </div>
          <button onClick={runPaymentTest}
            style={{ padding: '10px 22px', borderRadius: '10px', border: 'none', background: activeStatus?.color || '#3b82f6', color: '#fff', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', whiteSpace: 'nowrap' }}>
            <MdCreditCard size={18} /> Run Test
          </button>
        </div>

        {/* Result Panel */}
        {payTestResult && (() => {
          const res   = PAYMENT_STATUSES.find(s => s.id === payTestResult.status);
          const Icon  = res.icon;
          return (
            <div style={{ padding: '20px 24px', borderRadius: '16px', border: `2px solid ${res.color}`, background: res.bg, display: 'flex', alignItems: 'flex-start', gap: '18px' }}>
              <Icon size={32} style={{ color: res.color, flexShrink: 0, marginTop: '2px' }} />
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', flexWrap: 'wrap', gap: '8px' }}>
                  <strong style={{ fontSize: '1rem', color: res.color }}>{res.label}</strong>
                  <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontFamily: 'monospace' }}>{payTestResult.ts}</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', fontSize: '0.88rem' }}>
                  <div><span style={{ color: 'var(--text-muted)' }}>User</span><br /><strong>{payTestResult.user}</strong></div>
                  <div><span style={{ color: 'var(--text-muted)' }}>Amount</span><br /><strong>{payTestResult.amount}</strong></div>
                  <div><span style={{ color: 'var(--text-muted)' }}>Gateway</span><br /><strong>Stripe (Simulated)</strong></div>
                  <div><span style={{ color: 'var(--text-muted)' }}>Status code</span><br /><strong style={{ fontFamily: 'monospace', color: res.color }}>{payTestResult.status.toUpperCase()}</strong></div>
                </div>
                <p style={{ margin: '12px 0 0', fontSize: '0.83rem', color: 'var(--text-muted)' }}>{res.desc}</p>
              </div>
              <button onClick={() => setPayTestResult(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 0 }}>
                <MdClose size={18} />
              </button>
            </div>
          );
